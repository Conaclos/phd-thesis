...
Lorsqu'une valeur est insérée (respectivement supprimée) avant la $n$-ième valeur, cette dernière devient la $n+1$-ème valeur (respectivement la $n-1$-ème valeur).
...
Dans la lignée des travaux sur la réplication optimiste de séquences~\autocite{oster_2006_woot,sun_1998_cci}, nous soutenons qu'un pair ne souhaite pas insérer une valeur avant la $n$-ième valeur ou supprimer la $n$-ième valeur~:
...
Il génère d'abord un nouvel identifiant pour $v$ et récupère les identifiants de la $n-1$-ème valeur et de la $n$-ième valeur.
...
La $n-1$-ème valeur correspond à la valeur après laquelle $v$ doit être insérée.
...
L'intention d'insérer $v$ entre la $n-1$-ème valeur et la $n$-ième valeur est donc respectée à la fois par le référence à la $n-1$-ème valeur et par le choix des identifiants associés aux valeurs.
...
L'intégration d'un message d'insertion d'une valeur $v$ de position $p$ se déroule simplement par le placement de $v$ de manière à ce que les positions des valeurs qui précèdent $v$ dans la séquence soient inférieures à $p$ et les positions des valeurs qui succèdent $v$ soient supérieures à $p$.
...
\end{tikzpicture}
\caption[Exemple d'une séquence \emph{Treedoc}]{Exemple d'une séquence \emph{Treedoc} de valeurs associées à leur position (chemin dans l'arbre) et leur identifiant.
...
L'approche à granularité variable propose de permettre l'agrégation de valeurs et de leur position en blocs.
...
L'exécution de l'insertion d'une valeur $v$ à l'indice $j$ génère une position $p$ comprise entre les positions associées aux $j-1$-ème et $j$-ème valeurs de la séquence.
...
\begin{tikzpicture}
    ...
    \node (bot) at (mbot) {$\bot$};
    \node[
        label={[name=ccA3]above:$\set*{A_1,\dotsc,A_3}$}
    ] (cA3) at (mA3) {$\textit{lad}$};
    \node[
    ...
    \node[
        label={[name=ccA4r]above:$\set*{A_4}$}
    ] (cA4r) at (mA4r) {$\emptyset$};
    \node[
    ...
    \foreach \x in {A4,A4r,B1}
        \node[draw,rounded corners,fit=(c\x) (cc\x)](\x) {};

    \foreach \x in {A3,A3A4,A3B1,A4B1,A3A4r,A4rB1,A3A4B1,A3A4rB1}
        \node[fit=(c\x) (cc\x)](\x) {};
...
Nous prenons l'exécution abstraite de la \autoref{fig:replseq-sematic} pour illustrer la réplication d'une séquence qui utilise l'implémentation de la \autoref{fig:deltaseq} avec de positions \emph{dot}-ifiées.
La \autoref{fig:deltaseq-state-trans} décrit les états successifs de la copie du pair $p_A$.
La \autoref{fig:deltaseq-state-trans} décrit les états successifs de la copie du pair $p_B$.
Pour simplifier la représentation, les positions ne sont pas représentées.
À la place des associations de positions et valeurs, nous avons indiqué la séquence de valeur.
...
%         to +(0,-2*\hsep) node (s2) {$\tuple*{\textit{lady},\set*{A_1,\dotsc,A_4}}$}
%         to +(0,-3*\hsep) node (s3) {$\tuple*{\textit{ladiy},\set*{A_1,\dotsc,A_5}}$}
%         to +(0,-4*\hsep) node (s4) {$\tuple*{\textit{ladi},\set*{A_1,\dotsc,A_5}}$}
%         to +(0,-5*\hsep) node (s5) {$\tuple*{\textit{ladies},\set*{A_1,\dotsc,A_7}}$}
%         to +(0,-6*\hsep) node (s6) {$\tuple*{\textit{ladies},\set*{A_1,\dotsc,A_7}}$}
%     ;
%     % State transitions
%     \draw[pre] (s0) to node[midway,left]{initialise} (s1);
%     \draw[pre] (s1) to %
%             node[midway,left]{exécute $a_1$} %
%             node[midway,right]{génère $\tuple*{\textit{lady}, \set*{A_1,\dotsc,A_4}}$} %
%         (s2);
%     \draw[pre] (s2) to %
%             node[midway,left]{exécute $a_2$} %
%             node[midway,right]{génère $\tuple*{\textit{i}, \set*{A_5}}$} %
%         (s3);
%     \draw[pre] (s3) to %
%             node[midway,left]{exécute $a_3$} %
%             node[midway,right]{génère $\tuple*{\emptyset, \set*{A_4}}$} %
%         (s4);
%     \draw[pre] (s4) to %
%             node[midway,left]{exécute $a_4$} %
%             node[midway,right]{génère $\tuple*{\textit{es}, \set*{A_6,A_7}}$} %
%         (s5);
%     \draw[pre] (s5) to %
%             node[midway,left]{exécute $a_5$} %
%             node[midway,right]{retourne $\textit{ladies}$} %
%         (s6);
% \end{tikzpicture}
% \caption{États successifs de la copie du pair $p_A$ de l'exécution abstraite de la \autoref{fig:replseq-sematic}.
% L'ensemble répliqué utilise l'implémentation de la \autoref{fig:deltaseq}.}\label{fig:deltaseq-state-trans}
% \end{figure}
...
\begin{tikzpicture}
    \newcommand\hsep{4}
    % Events
    \node (s1) {$\tuple*{\emptyset, \emptyset}$}
        to +(0,-\hsep) node (s2) {\begin{tikzpicture}
            \node[block,
                    label={below:$f^{A}_{\irange{1}{4}}$},
                    label={[label distance=6mm]right:$\set*{A_1,\dotsc,A_4}$}
                ] {$lady$}
            ;
        \end{tikzpicture}}
        to +(0,-2*\hsep) node (s3) {\begin{tikzpicture}
            \node[block,
            ...
            to +(\widthblock+3*\widthletter,0) node[anchor=west]{$\set*{A_1,\dotsc,A_5}$}
            ;
        \end{tikzpicture}}
        to +(0,-3*\hsep) node (s4) {\begin{tikzpicture}
            \node[block,
                    label={below:$f^{A}_{\irange{1}{3}}$}
                ] {$lad$}
            to ++(\widthblock,0) node[letter,
            ...
            ;
        \end{tikzpicture}}
        to +(0,-4*\hsep) node (s5) {\begin{tikzpicture}
            \node[block,
                    label={below:$f^{A}_{\irange{1}{3}}$}
                ] {$lad$}
            to ++(\widthblock,0) node[block,
            ...
            ;
        \end{tikzpicture}}
        to +(0,-5*\hsep) node (s6) {\begin{tikzpicture}
            \node[block,
                    label={below:$f^{A}_{\irange{1}{3}}$}
                ] {$lad$}
            to ++(\widthblock,0) node[block,
...
\begin{tikzpicture}[every node/.style={}]
    \newcommand*\hsep{1.4}
    \newcommand*\vsepevt{-3.4}
    \path (0,0)
        to +(-4,0) node[
            label={above:\textbf{(1) Initialise}}
        ]{\begin{tikzpicture}
            \node[label={[label distance=6mm]right:$\emptyset$}
                ] {$\emptyset$}
            ;
        \end{tikzpicture}}
        to +(-4,\vsepevt) node[
        ...
        to +(-4,2*\vsepevt) node[
            ...
            to +(\widthblock+\widthletter,0) node[letter,
                    label={below:$f^A_5$},
                    label={[label distance=6mm]right:$\set*{A_1,\dotsc,A_5}$}
                ]{$y$}
            ;
        \end{tikzpicture}}
        to +(4,2*\vsepevt) node[
            ...
            \node[letter,
                    label={below:$f^A_5$},
                    label={[label distance=6mm]right:$\set*{A_5}$}
                ]{$i$}
            ;
        \end{tikzpicture}}
        to +(-4,3*\vsepevt) node[
        ...
        to +(4,3*\vsepevt) node[
            ...
            \node[label={[label distance=6mm]right:$\set*{A_4}$}
                ]{$\emptyset$}
            ;
        \end{tikzpicture}}
        to +(-4,4*\vsepevt) node[
            label={above:\textbf{(4) Exécute $\trm{ins}(4, \textit{es})$}}
        ]{\begin{tikzpicture}
            \node[block,
                    label={below:$f^{A}_{\irange{1}{3}}$}
                ] {$lad$}
            to ++(\widthblock,0) node[block,
                    label={below:$f^A_3 q^A_{\irange{5}{7}}$},
                    label={[label distance=6mm]right:$\set*{A_1,\dotsc,A_7}$}
                ]{$ies$}
            ;
        \end{tikzpicture}}
        to +(4,4*\vsepevt) node[
            ...
            \node[block,
                    label={below:$f^A_3 q^A_{\irange{5}{7}}$},
                    label={[label distance=6mm]right:$\set*{A_6,A_7}$}
                ]{$es$}
            ;
        \end{tikzpicture}}
    ;
...
\begin{tikzpicture}[every node/.style={}]
    ...
    \path (0,0)
        ...
        to +(-4,2*\vsepevt) node[
            ...
            to +(\widthblock,0) node[letter,
                    label={below:$t^B_1$},
                    label={[label distance=6mm]right:$\set*{A_1,\dotsc,A_4,B_1}$}
                ]{$\negthinspace!$}
            ;
        \end{tikzpicture}}
        to +(4,2*\vsepevt) node[
            ...
            \node[letter,
                    label={below:$t^B_1$},
                    label={[label distance=6mm]right:$\set*{B_1}$}
                ]{$\negthinspace!$}
            ;
        \end{tikzpicture}}
        to +(-4,3*\vsepevt) node[
            ...
            to +(2*\widthblock,0) node[letter,
                    label={below:$t^B_1$},
                    label={[label distance=6mm]right:$\set*{A_1,\dotsc,A_7,B_1}$}
                ]{$\negthinspace!$}
            ;
        \end{tikzpicture}}
        to +(4,3*\vsepevt) node[
            label={above:\textbf{Message $m_2$}}
        ]{\begin{tikzpicture}
            \node[block,
                    label={below:$f^A_3 q^A_{\irange{5}{7}}$},
                    label={[label distance=6mm]right:$\set*{A_4,\dotsc,A_7}$}
                ]{$ies$}
            ;
        \end{tikzpicture}}
        to +(-4,4*\vsepevt) node[
            label={above:\textbf{(4) Intègre $m_1$}}
        ]{\begin{tikzpicture}
            \node[block,
                    label={below:$f^{A}_{\irange{1}{3}}$}
                ] {$lad$}
...
\begin{tikzpicture}
    ...
    \node (s0) {}
        to +(0,-\hsep) node (s1) {$\tuple*{\emptyset, \emptyset}$}
        to +(0,-2*\hsep) node (s2) {$\tuple*{\textit{lady},\set*{A_1,\dotsc,A_4}}$}
        to +(0,-3*\hsep) node (s3) {$\tuple*{\textit{lady\textbf{!}},\set*{A_1,\dotsc,A_4,B_1}}$}
        to +(0,-4*\hsep) node (s4) {$\tuple*{\textit{lady\textbf{!}},\set*{A_1,\dotsc,A_4,B_1}}$}
        to +(0,-5*\hsep) node (s5) {$\tuple*{\textit{ladies\textbf{!}},\set*{A_1,\dotsc,A_7,B_1}}$}
        to +(0,-6*\hsep) node (s6) {$\tuple*{\textit{ladies\textbf{!}},\set*{A_1,\dotsc,A_7,B_1}}$}
        to +(0,-7*\hsep) node (s7) {$\tuple*{\textit{ladies\textbf{!}},\set*{A_1,\dotsc,A_7,B_1}}$}
    ;
    % State transitions
    \draw[pre] (s0) to node[midway,left]{initialise} (s1);
    \draw[pre] (s1) to %
            node[midway,left]{intègre $\tuple*{\textit{lady}, \set*{A_1,\dotsc,A_4}}$} %
        (s2);
    \draw[pre] (s2) to %
            node[midway,left]{exécute $b_1$} %
            node[midway,right]{génère $\tuple*{\textit{\textbf{!}},\set*{B_1}}$} %
        (s3);
    \draw[pre] (s3) to %
            node[midway,left]{exécute $b_2$} %
            node[midway,right]{retourne $\textit{lady\textbf{!}}$} %
        (s4);
    \draw[pre] (s4) to %
            node[midway,left]{intègre $\tuple*{\textit{ies}, \set*{A_4,\dotsc,A_7}}$} %
        (s5);
    \draw[pre] (s5) to %
            node[midway,left]{intègre $\tuple*{\textit{lady}, \set*{A_1,\dotsc,A_4}}$} %
        (s6);
    \draw[pre] (s6) to %
            node[midway,left]{exécute $b_3$} %
            node[midway,right]{retourne $\textit{ladies\textbf{!}}$} %
        (s7);
\end{tikzpicture}
\caption{États successifs de la copie du pair $p_B$ de l'exécution abstraite de la \autoref{fig:replseq-sematic}.
L'ensemble répliqué utilise l'implémentation de la \autoref{fig:deltaseq}.}\label{fig:deltaseq-state-trans}
\end{figure}
