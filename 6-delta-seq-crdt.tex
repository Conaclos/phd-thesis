...
L'utilisation de l'implémentation de la \autoref{fig:deltaseq} et de ce nouveau type de positions permet la réplication d'une séquence \emph{Dotted LogootSplit} synchronisée par différences d'état.
...
Le pair $p_A$ initialise sa séquence et exécute une succession d'opérations qui génère des messages.
...
Nous illustrons en partie cette liberté à l'aide de la succéssion des états du pair $p_B$.
...
Ces messages sont perdus sur le réseau.
...
Il reçoit ensuite une différence d'état dont l'intégration conduit à la suppression du $\textsf{'\textit{y}'}$ (4).
$p_A$ détecte la perte de messages sur le réseau et transmet alors une différences d'état qui provient de la fusion des dernières différences d'états qu'il a transmis (y compris celle qui conduit à la suppression de  $\textsf{'\textit{y}'}$).
$p_B$ intègre l'état qu'il reçoit et obtient ainsi la séquence $\textsf{'\textit{ladies\textbf{\negthinspace!}}'}$.
...
La séquence obtenue est relativement proche de celle obtenue avec \emph{LogootSplit}.
...
\emph{Dotted LogootSplit} a également moins de suppositions sur la livraison de messages.
Ce qui permet la conception de protocoles de synchronisation plus souples et mieux adaptés au condition réseau ou au type de collaboration rencontré.
...
\begin{tikzpicture}[every node/.style={}]
    ...
    ;
\end{tikzpicture}
\caption[États successifs de la copie du pair $p_A$ de l'exécution abstraite de la \autoref{fig:replseq-sematic-1}]{États successifs de la copie du pair $p_A$ de l'exécution abstraite de la \autoref{fig:replseq-sematic-1}.
...
\begin{tikzpicture}[every node/.style={}]
    ...
    ;
\end{tikzpicture}
\caption[États successifs de la copie du pair $p_B$ de l'exécution abstraite de la \autoref{fig:replseq-sematic-1}]{États successifs de la copie du pair $p_B$ de l'exécution abstraite de la \autoref{fig:replseq-sematic-1}.
