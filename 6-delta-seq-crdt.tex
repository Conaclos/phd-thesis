...
Ces observations nous conduisent à formuler la question de recherche suivante~: un protocole synchronisé par différences d'états pour l'édition collaborative en simultané de texte est-il envisageable~?
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \path node (A) {$p_A$};
        % Timeline ends
        \path (6*\hsep+1.2,0) coordinate (aend);
        % Events
        \path (A)
            to +(\hsep,0) node[
                label=above:{$\trm{ins}(0, \textit{citi})$}
            ] (a1) {$a_{\irange{1}{4}}$}
            to +(2*\hsep,0) node[
                label=above:{$\trm{rd}\textit{cit\textbf{i}}$}
            ] (a3) {$a_5$}
            to +(3*\hsep,0) node[
                label=above:{$\trm{del}(3)$}
            ] (a4) {$a_6$}
            to +(4*\hsep,0) node[
                label=above:{$\trm{rd}\textit{cit}$}
            ] (a5) {$a_7$}
            to +(5*\hsep,0) node[
                label=above:{$\trm{ins}(3, \textsf{'\textit{y}'})$}
            ] (a6) {$a_8$}
            to +(6*\hsep,0) node[
                label=above:{$\trm{rd}\textit{cit\textbf{y}}$}
            ] (a7) {$a_9$}
        ;
        % Timelines
        \foreach \src/\dest in {A/a1,a7/aend}
            \draw[timeline] (\src) to (\dest);
        % Precdence
        \foreach \src/\dest in {a1/a3,a3/a4,a4/a5,a5/a6,a6/a7}
...
\begin{subfigure}{\linewidth}
    ...
    \begin{tikzpicture}
        ...
        \path (A)
            ...
            to +(10*\hsep,0) node[
                label={above:$\trm{rd} \textit{lad\textbf{ies}}$}
            ] (a8) {$a_8$}
        ;
        \path (B)
            to +(3*\hsep,0) node[
        ...
        ;
        % Timelines
        \foreach \src/\dest in {A/a1,a8/aend,B/b1,b3/bend}
            \draw (\src) edge[timeline] (\dest);
        % Precdence
        \foreach \src/\dest in {a1/a5,a5/a6,a6/a7,a7/a8,b1/b2,b2/b3}
            \draw[pre] (\src) to (\dest);
        \foreach \src/\dest in {a1/b1,a8/b3}
            \draw[pre] (\src) to (\dest);
    \end{tikzpicture}
    \caption{}
    \label{fig:replseq-sematic-1}
\end{subfigure}
\par\medskip
...
\subref{fig:replseq-sematic-4} Les valeurs de retour admises de l'opération de lecture $a_3$ dépendent de la spécification choisit.
Elles correspondent à des extensions linéaires des ordres partiels induits par la spécification faible \subref{fig:replseq-sematic-2} ou la spécification forte \subref{fig:replseq-sematic-3}.}
...
\subref{fig:woot-2} Valeurs référencés par les valeurs de la séquence.
    Les références vers le début et la fin de la séquence sont omises.}
\label{fig:woot}
\end{figure}
...
Les références vers le début de la séquence sont omises.}
\label{fig:rga}
\end{figure}
...
\begin{subfigure}[b]{0.69\linewidth}
    ...
    \begin{tikzpicture}
        ...
        to ++(\widthletter,0) node[letter,
                label={below:$l_A$},
                label={[label distance=6mm]below:$q_A5$}
            ]{$i$}
        to ++(\widthletter,0) node[letter,
                label={below:$p_A6$}
            ]{$e$}
...
\subref{fig:logoot-2} Valeur supprimée associée à sa position.}
\label{fig:logoot}
\end{figure}
...
\begin{subfigure}[b]{0.69\linewidth}
    ...
    \begin{tikzpicture}
        ...
        to ++(\widthblock,0) node[block,
                label={below:$f^{A1}_2$},
                label={[label distance=6mm]below:$q^{A2}_{\irange{0}{2}}$}
            ]{$ies$}
...
\subref{fig:logootsplit-2} Valeur supprimée associée à sa position.}
...
Dans un premeir temps nous présentons u nformalisme qui permet de décrire de spositions densément ordonnées.
...
Deux positions sont agréables, si et seulement si \begin{inlinelist}\item elles divergent uniquement par leur identifiant local (le nombre séquentiel du dernier n-uplet), et \item elles ont des identifiants locaux contiguës\end{inlinelist}.
...
\begin{subfigure}[b]{0.69\linewidth}
    ...
    \begin{tikzpicture}
        ...
        to ++(\widthblock,0) node[block,
                label={below:$f^{A}_3$},
                label={[label distance=6mm]below:$q^{A}_{\irange{5}{7}}$}
            ]{$ies$}
        to ++(\widthblock,0) node[letter,
                label={below:$t^{B}_1$}
            ]{$\negthinspace!$}
...
\subref{fig:logootsplit-1} Séquence de valeurs associées à leurs positions.
\subref{fig:logootsplit-2} Contexte causal de la séquence.}
