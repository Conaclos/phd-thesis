...
\begin{tikzpicture}
    ...
    \foreach \src/\dest in {A/a1,a3/aend,B/b1,b1/bend}
        \draw (\src) edge[timeline] (\dest);
    % Precedence
    \foreach \src/\dest in {a1/a2,a2/a3,b1/a3}
        \draw[pre] (\src) to (\dest);
\end{tikzpicture}
...
\begin{tikzpicture}
    ...
    \draw[pre] (s1) to %
            ...
            node[midway,right]{génére $\tuple*{\trm{add},\tuple*{1,A_1}}$} %
        (s2);
    \draw[pre] (s2) to %
            node[midway,left]{exécute $a_2$} %
            node[midway,right]{génére $\tuple*{\trm{rmv},\set*{A_1}}$} %
        (s3);
    \draw[pre] (s3) to %
...
L'ensemble des états équipé de la fonction d'intégration forme un sup-demi-treillis~\autocite{davey2002lattice}.
...
\begin{tikzpicture}
    ...
    \draw[pre] (s1) to %
            node[midway,left]{exécute $a_1$} %
            node[midway,right]{génére $\tuple*{\set*{\tuple*{1,A_1}},\set*{A_1}}$} %
        (s2);
    \draw[pre] (s2) to %
            node[midway,left]{exécute $a_2$} %
            node[midway,right]{génére $\tuple*{\emptyset,\set*{A_1}}$} %
        (s3);
    \draw[pre] (s3) to %
            node[midway,left]{intègre $\tuple*{\set*{\tuple*{1,B_1}},\set*{B_1}}$} %
        (s4);
    \draw[pre] (s4) to %
            node[midway,left]{exécute $a_3$} %
            node[midway,right]{retourne $\set*{1}$} %
...
\begin{tikzpicture}
    ...
    \node[draw,rounded corners] (A2) at (\hsep,\vsep) {$\set*{\tuple*{1,B_1}}, \set*{B_1}$};
    \node[draw,rounded corners] (A1p) at (-2*\hsep,2*\vsep) {$\emptyset, \set*{A_1}$};
    \node[draw,rounded corners] (A2p) at (2*\hsep,2*\vsep) {$\emptyset, \set*{B_1}$};
    \node (A1A2) at (0,2*\vsep) {$\set*{\tuple*{1,A_1}, \tuple*{1,B_1}}, \set*{A_1, B_1}$};
    \node (A1pA2) at (-\hsep,3*\vsep) {$\set*{\tuple*{1,B_1}}, \set*{A_1,B_1}$};
    \node (A1A2p) at (\hsep,3*\vsep) {$\set*{\tuple*{1,A_1}}, \set*{A_1,B_1}$};
    \node (A1pA2p) at (0,4*\vsep) {$\emptyset, \set*{A_1,B_1}$};
    \draw (bot) to (A1);
    \draw (bot) to (A2);
    \draw (A1) to (A1p);
    \draw (A2) to (A2p);
