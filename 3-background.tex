...
Les attentes des collaborateur·ice·s sont exprimées sous la forme de propriétés de cohérence\index{properiete-coherence@Propriété de cohérence}.
...
Les collaborateur·ice·s interagissent avec le système en exécutant des opérations\index{op@Opérations exécutées}.
L'ensemble des opérations exécutées forme une \emph{histoire}\index{hstoire@Histoire}.
...
\paragraph{Retourne-avant~\autocite{burckhardt_eventualconsistency_2014}.}\index{relation-retourne-avant@Relation retourne-avant} Une relation d'ordre partiel qui rend compte des opérations dont l'exécution ne coïncide pas dans le temps.
...
\begin{definition}[histoire]\label{def:history}~\autocite{burckhardt_eventualconsistency_2014}
...
\paragraph{Visibilité~\autocite{burckhardt_eventualconsistency_2014}.}\index{relation-visibilite@Relation de visibilité} Une relation qui rend compte de l'intégration des effets des opérations de modification par les pairs.
...
\paragraph{} Une histoire augmentée de la relation de \emph{visibilité} consiste en une \emph{exécution abstraite}~\autocite{burckhardt_eventualconsistency_2014}\index{execuction-abstraite@Exécution abstraite}.
...
\begin{definition}[exécution abstraite]\label{def:abstract-exec}~\autocite{burckhardt_eventualconsistency_2014}
...
Le contexte d'une opération\index{context-operation@Contexte d'une opération} regroupe les opérations de modification qui lui sont visibles.
...
Nous qualifions ces pairs de \emph{malintentionnés}\index{pairs-malintentionnes@Pairs malintentionnés}.
Inversement, les pairs qui suivent le protocole sont qualifiés d'\emph{honnêtes}\index{pairs-honnetes@Pairs honnêtes}.
...
Une histoire qui remplie cette condition est une \emph{histoire bien-formée}\index{histoire-bien-formee@Histoire bien-formée}.
...
La \autoref{def:op-obs} définit ce qu'est un observateur d'une opération\index{observateurs-operations@Observateurs d'une opération}.
...
Une \emph{exécution abstraite bien-formée}\index{execuction-abstraite-bien-formee@Exécution abstraite bien-formée} inclut uniquement les opérations qui sont observées par les pairs honnêtes.
...
\index{propriete-vivacite@Propriétés de vivacité}
\index{propriete-surete@Propriétés de sûreté}
...
Ces histoires hypothétiques sont des \emph{extensions de l'histoire}\index{extension-histoire@Extension d'une histoire}.
...
Un protocole garantit un \emph{modèle de cohérence}\index{model-coherence@Modèle de cohérence} si toutes les histoires qu'il peut produire peuvent être justifiées par au moins une \emph{exécution abstraite} qui respecte ce modèle de cohérence.
...
Nous pouvons néanmoins juger de la \enquote{force}\index{force-modele-coherence@Force des modèles de cohérence} d'un modèle de cohérence par rapport au nombre d'exécutions abstraites qu'il accepte.
...
\index{coherence-a-termes@Cohérence à terme forte}
...
\index{coherence-causal@Cohérence causale}
...
La transitivité de la relation de visibilité nous permet de définir la relation de \emph{concurrence}\index{relation-concurrence@Relation de concurrence} et la relation de \emph{visibilité immédiate}\index{relation-visibilite-immediate@Relation de visibilité immédiate}.
...
\begin{definition}[Cohérence causale non-spéculative]\label{def:causal-consistency}~\autocite{mahajan_2011_cac}
...
\index{coherence-fjc@Cohérence \acl{FJC}}
...
\begin{definition}[Cohérence \emph{\acl{FJC}} non-spéculative]\label{def:fjc-consistency}~\autocite{mahajan_2011_cac}
...
\index{coherence-vfjc@Cohérence \acl{VFJC}}

Les pairs malintentionnés peuvent réaliser des équivoques.
Dans une exécution abstraite, ces équivoques se traduisent par des opérations non-linéaires\index{operation-non-lineaire@Opérations non-linéaires}.
Une opération est non-linéaire s'il existe une opération du même auteur qui lui est concurrente.
Chaque opération non-linéaire forme un embranchement\index{embranchement@Embranchement}.
...
Un pair reconnaît qu'un pair est malintentionné\index{pair-reconnu-malintentionne@Pairs reconnus malintentionnés} dès lors qu'il observe deux opérations non-linéaires de ce dernier.
...
\begin{definition}[Cohérence \emph{\acl{VFJC}}]\label{def:vfjc-consistency}~\autocite{mahajan_2011_cac}
...
Les \acfp{CRDT}~\autocite{shapiro_2011_crdt,roh_2011_rga}\index{crdt@\aclp{CRDT}} sont des types de données conçus pour être répliqués et modifiés en parallèle par un ensemble de pairs.
...
\index{type-abstrait-donnees-repliquees@Type abstrait de données répliquées}
...
\index{schema-synchronisation@Schéma de synchronisation}
...
Pour décrire l'implémentation de ces ensembles répliqués, nous introduisons un formalisme inspiré des travaux de \textcite{baquero_2018_pure-op-crdt}.
...
\paragraph{Les \acp{CRDT} synchronisés par opérations~\autocite{baquero_2014_pure-op-crdt,baquero_2018_pure-op-crdt}}\index{crdt-op@\acp{CRDT} synchronisés par opérations} représentent les modifications effectuées sur une copie par des opérations intégrables.
...
\paragraph{Les \acp{CRDT} synchronisés par états}\index{crdt-etats@\acp{CRDT} synchronisés par états} transmettent directement l'état mis à jour au reste des pairs.
...
En effet, l'ensemble des états équipé de la fonction d'intégration forme un sup-demi-treillis~\autocite{davey2002lattice}\index{sup-demi-treillis@Sup-demi-treillis}.
...
\paragraph{Les \acp{CRDT} synchronisés par différences}\index{crdt-diff@\acp{CRDT} synchronisés par différences}~\autocite{almeida_2018_delta-crdt-revisited} sont une sous-famille des \acp{CRDT} synchronisés par états.
...
Ils reposent sur l'idée de décomposer un état en un ensemble d'états irréductibles\index{etats-irreductibles@États irréductible}.
