...
Alice lit le second message avant de lire le premier.}
\label{fig:causal-expectation}
\end{figure}
...
Ici, cette information est explicitement marquée par l'utilisation de la relation \emph{retourne-avant} $\rb$.}
\label{fig:rb-example}
\end{figure}
...
\begin{table}[ht]
    ...
    Les valeurs de retour correspondent à des ensembles de messages $\trm{Val}_T = \powerfset{\trm{String}}$. $\powerfset{\trm{String}}$ est l'ensemble des sous-ensembles finis des chaînes de caractères.}
    \label{tab:op-attributes}
\end{table}
...
Le protocole $P$ garantit ainsi la propriété $p_1$ puisque chcune de ses histoires est associée à au moins une exécution abstraite qui la respecte.}
\label{fig:hist-absexec}
\end{figure}

\begin{definition}[exécution abstraite]\label{def:abstract-exec}
...
\begin{table}[htb]
    ...
    \end{tabular}
    \caption{Contexte et observateurs des opérations de l'exécution abstraite de la~\autoref{fig:vis-example}.}
    \label{tab:op-ctx-obs}
\end{table}
...
\subref{fig:no-enum-example-2} Exemple d'une exécution abstraite correspondante.}
\label{fig:no-enum-example}
\end{figure}
...
Elle ne respecte pas la propriété~\ref{itm:bf2}. L'opération $m_1$ est uniquement observée par un pair mal-intentionné $p_M$.
L'exécution abstraite \subref{fig:wf-example-2} est bien-formée.
L'opération $m_1$ est observée par le pair honnête $p_A$.}
...
\begin{definition}[Modèle de cohérence]\label{def:consistency-model}
...
\label{fig:causal-fjc-vfjc-order}
\end{figure}
...
Dans une exécution abstraite qui respecte le modèle de cohérence causale, si une opération $x$ cause potentiellement une opération $y$, alors $x$ est intégrée avant l'exécution de $y$.
...
Nous disons que deux opérations $x$ et $y$ sont concurrentes, et nous écrivons $x \concur y$, si $x$ ne précède pas $y$ et $y$ ne précède pas $x$.
...
\begin{subfigure}{\linewidth}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:vis-vs-pre-2}
\end{subfigure}
\caption{Exécutions abstraites qui respectent le modèle de cohérence causale.}
\label{fig:vis-vs-pre}
\end{figure}
...
\begin{definition}[Cohérence causale non-spéculative]\label{def:causal-consistency}
...
Il prétend au pair $p_A$ que sa deuxième opération est $m_2$, alors qu'il prétend au pair $p_B$ que sa deuxième opération est $m_3$.}
\label{fig:causal-violation}
\end{figure}
...
\begin{definition}[Cohérence \emph{\acl{FJC}} non-spéculative]\label{def:fjc-consistency}
  ...
  \begin{description}
  \item[\namedlabel{itm:vis-trand}{C1} (visibilité transitive)]
  La relation de visibilité est transitive.
  \begin{equation*}
    x \vis y \land y \vis z \implies x \vis z
  \end{equation*}
...
%Leur résolution peut notamment produire un sur-coût [CITATIION: Depot?].
...
\begin{tikzpicture}
    ...
    \draw[pre,bend right=20] (m1) to (m3);
\end{tikzpicture}
\caption{Exemple d'exécutions abstraites qui respectent le modèle de cohérence \ac{VFJC}.}
\label{fig:provably-malicious}
\end{figure}
...
\begin{table}[htb]
    ...
    \begin{tabular}{c|c}
        ...
        $b_3$ & $\set*{p_M}$ \\
    \end{tabular}
    \caption{Pairs reconnus mal-intentionnés dans l'exécution abstraite de la~\autoref{fig:provably-malicious} par le pair qui exécute l'opération considérée.}
    \label{tab:known-malicious}
\end{table}
...
L'excéutio nabstarite $A_1$ respecte en revanche le modèle de cohérence \ac{VFJC}.}
\label{fig:known-colluding-malicious}
\end{figure}
...
\begin{definition}[Cohérence \emph{\acl{VFJC}}]\label{def:vfjc-consistency}
...
L'exécution abstraite respecte le modèle de cohérence causale.}
\label{fig:set-conflict-example}
\end{figure}
...
Le type abstrait de données répliquées définit les op"rations qui peuvent être exécutées et leur effet lorsqu'elles sont exécutées en parallèle ou en séquence.
...
\subsection{Type abstrait de données répliquées}
\label{sec:crdt-spec}
...
\begin{figure}[tb]
    ...
    \end{align*}
    \caption{Spécification algébrique du type abstrait de données usuel Ensemble.}
    \label{fig:set-spec}
\end{figure}
...
\begin{subfigure}{\linewidth}
    ...
    \caption{}
\end{subfigure}
\caption[Impact d'ordres distincts d'intégration des messages sur un ensemble répliqué]{Exécutions abstraites qui rend compte des ordres d'intégrations des opérations de l'exécution abstraite de la~\autoref{fig:set-conflict-example}.}
\label{fig:spec-linearization}
\end{figure}
...
Contrairement aux type abstraits usuels, plusieurs sémantiques peuvent se révéler utiles et dépendre de leur domaine d'application.
...
%    &\trm{cl}(x) \defeq \trm{max}(\set*{1 + \trm{cl}(y) \given y \vis x \land \trm{call}(y) = \trm{rmv}(v)})
%\end{split}\end{equation}
%
%\begin{equation}\begin{split}
%\MoveEqLeft\forall v \qsep x \in A \land x = \trm{rmv}(v) \implies\\
%    &\trm{cl}(x) \defeq \trm{max}(\set*{1 + \trm{cl}(y) \given y \vis x \land \trm{call}(y) = \trm{add}(v)})
%\end{split}\end{equation}
%
%\begin{equation}\begin{split}
%\MoveEqLeft x \in A \land \trm{call}(x) = \trm{rd} \implies \trm{rval}(x) =\\
...
\subsection{Schéma de synchronisation}
\label{sec:crdt-sync}
...
Lorsqu'un pair exécute une opération d'interrogation, il évalue la valeur de retour de cette interrogation à l'aide
...
Le raffinement des \acp{CRDT} synchronisés par opérations z conduit à la formulation d'une sous-famille qualifiée de pure~\autocite{baquero_2014_pure-op-crdt,baquero_2018_pure-op-crdt}.
...
%Un \ac{CRDT} synchronisé par opérations peut être représenté comme un \ac{CRDT} équipé d'un ensemble de requêtes d'opérations $Q^{\trm{op}}$ et d'une fonction $\trm{effect}$.
%Les requêtes d'opérations permettent de dériver une opération qui peut être jouée sur toutes les copies par l'intermédiaire de la fonction $\trm{effect}$.
%Chaque modificateurs du \ac{CRDT} est associé à une requête d'opération qui prend les mêmes types de paramètres en entrée, mais donne une opération au lieu d'une version mis à jour de l'état.
%La fonction $\trm{effect}$ prend respectivement un état et une opération comme premier et second paramètres, et retourne un état mis à jour.
%Dans la littérature~\autocite{baquero_2018_pure-op-crdt}, l'ordre des paramètres de la fonction $\trm{effect}$ sont inversés, et les requêtes d'opérations sont substituées à une unique fonction nommée $\trm{prepare}$.
%Nous avons décidé de nous écarter de ces choix pour exprimer des invariants plus concis entre les modificateurs et leurs requêtes d'opérations.
%Pour ce faire nous utilisons des applications partielles de fonction, ainsi que des compositions de fonctions.
%\begin{align*}
%&\forall S \in \trm{OpCRDT} \qsep \tuple*{S, Q^{\trm{op}}, \trm{effect}}\\
%&S \in \trm{CRDT}\\
%&\forall m \in M \qsep \exists q^{\trm{op}} \in Q^{\trm{op}} \qsep \:\forall i \in \mathbb{I} \qsep m_i(s) = \trm{effect}_i(s) \circ q^{\trm{op}}_i(s)
%\end{align*}
...
L'intégration de la suppression d'une valeur est effectuée après l'intégration de son ajout.}
\label{fig:op-add-win-set}
\end{figure}
...
%\begin{align*}
%&\forall S \in \trm{StateCRDT} \qsep \tuple*{S, \bot, \sqcup}\\
%&S \in \trm{CRDT}\\
%&\tuple*{S, \bot, \sqcup}\ \textit{est un sup-demi-treillis} \\
%&\forall m \in M \:\forall s \in S\ \forall d \in \trm{dom}(m(s)) \qsep m(s)(d) \sqsubseteq s
%\end{align*}
...
\MoveEqLeft\trm{prepare}_i(\tuple*{m, c}, \trm{add}(v)) \defeq \tuple*{m \cup \set*{\tuple*{v, i, n}},c \cup \tuple*{i, n}}\\
    &\where n \defeq 1 + \trm{max}(\set*{0,n \given \tuple*{i, n} \in c})
\end{aligned}\\
...
\end{aligned}
\end{align*}
\caption[Implémentation d'un ensemble répliqué synchronisé par états]{Implémentation d'un ensemble répliqué synchronisé par états qui respecte la sémantique \emph{Add-Win}.}
\label{fig:state-add-win-set}
\end{figure}

La \autoref{fig:state-add-win-set} présente une implémentions d'un ensemble répliqué synchronisé par états.
...
\begin{tikzpicture}
    ...
    \newcommand\vsep{1.3}
    \node (bot) at (0,0) {$\bot$};
    \node[draw,rounded corners] (A1) at (-\hsep,\vsep) {$\set*{\tuple*{1,A_1}}, \set*{A_1}$};
    \node[draw,rounded corners] (A2) at (\hsep,\vsep) {$\set*{\tuple*{1,A_2}}, \set*{A_2}$};
    \node[draw,rounded corners] (A1p) at (-2*\hsep,2*\vsep) {$\emptyset, \set*{A_1}$};
    \node[draw,rounded corners] (A2p) at (2*\hsep,2*\vsep) {$\emptyset, \set*{A_2}$};
    \node (A1A2) at (0,2*\vsep) {$\set*{\tuple*{1,A_1}, \tuple*{1,A_2}}, \set*{A_1, A_2}$};
    \node (A1pA2) at (-\hsep,3*\vsep) {$\set*{\tuple*{1,A_2}}, \set*{A_1,A_2}$};
    \node (A1A2p) at (\hsep,3*\vsep) {$\set*{\tuple*{1,A_1}}, \set*{A_1,A_2}$};
    \node (A1pA2p) at (0,4*\vsep) {$\emptyset, \set*{A_1,A_2}$};
    \draw (bot) to (A1);
    \draw (bot) to (A2);
    \draw (A1) to (A1p);
    \draw (A2) to (A2p);
    ...
    \draw (A1A2) to (A1pA2);
    \draw (A1A2) to (A1A2p);
    \draw (A1pA2) to (A1pA2p);
    \draw (A1A2p) to (A1pA2p);
\end{tikzpicture}
...
%$D(s)$ représente la famille de décompositions de l'état $s$.
...
%Un \ac{CRDT} synchronisé par différences d'états est un \ac{CRDT} synchronisé par état équipé d'un ensemble de modificateurs delta $M^\delta$.
%Chaque modificateur est associé à un modificateur delta et peut être définit avec ce dernier.
%Un modificateur delta retourne l'état minimal $\bot$ ou un état $e$.
%Prenant $j$ un état irréductible qui compose $e$, la fusion de $j$ avec l'état sur lequel la modification a été appliquée doit produire une inflation.
%Cette propriété n'est pas requise dans la définition originale des modificateurs delta.
%Elle a été introduire ultérieurement~\autocite{enes_2018_efficient-sync-state-based-crdt} sous le nom de modificateurs delta minimaux.
%\begin{align*}
%&\forall S \in \trm{DeltaCRDT} \qsep \tuple*{S, M^\delta}\\
%&S \in \trm{StateCRDT}\\
%&\forall m \in M \qsep \exists m^\delta \in M^\delta \qsep m(s) = s \sqcup m^\delta(s)\\
%&\begin{aligned}
%    \MoveEqLeft\forall s \in S \:\forall m^\delta \in M^\delta \:\forall d \in \trm{dom}(m^\delta(s))\\
%    &\exists J \in D(m^\delta(s)(d)) \:\forall j \in J \qsep j \not\sqsubseteq s
%\end{aligned}
%\end{align*}
...
\end{aligned}
\end{align*}
\caption[Implémentation d'un ensemble répliqué synchronisé par différences d'états]{Implémentation d'un ensemble répliqué synchronisé par différences d'états qui respecte la sémantique \emph{Add-Win}.}
\label{fig:delta-add-win-set}
\end{figure}
...
\paragraph{} Nous avons présentés les deux principales familles de \acp{CRDT}~: les \acp{CRDT} synchronisés par opérations et les \acp{CRDT} synchronisés par états.
