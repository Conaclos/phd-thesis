...
Elle définit en particulier les acractéristiques associées aux opérations.
...
Il s'agit d'un ordre partiel localement fini\footnote{Pour tout couple d'opérations, l'ensmeble des opérations comprises entre ces 2 opérations est fini.} qui peut être interprété sur une ligne de temps\footnote{\textcite{greenough_1976_semiorder} parle d'ordre d'intervalle.}~:
\begin{equation*}
...
La \autoref{def:abstract-exec} définit formellemnt une exécution abstraite.
La \autoref{fig:vis-example} est une exécution abstraite de l'hésitoire de la \autoref{fig:causal-expectation}.
...
La spécification du type de données répliquées de la~\autoref{fig:causal-expectation} (fil de discussion) peut ainsi être exprimé de la manière suivante~:
...
L'union de ces deux ensemble correpond à l'ensemble des pairs $\trm{Peers}$.
...
La \autoref{tab:op-ctx-obs} donne les observateurs de chaque opérations de l'histoire de la \autoref{fig:causal-expectation}.
...
La \autoref{fig:wf-example} donne un exemple d'exécution abstraite mal-gformée et d'une exécution abstraite bien-formée.
...
Par abus de langage nousd ésignons l'ensemble des exécutions abstraites qui respectent un modèle de cohérence par le nom du modèle de cohérecne.
...
Dans la \autoref{def:consistency-hierearchy}, nous introduisons la notation que nous utilisons pour mettre en évidence qu'un modèle de cohérecne est plus fort qu'un autre.
...
La \autoref{fig:vis-vs-pre-1} et la \autoref{fig:vis-vs-pre-2} sont équiavlentes.
La représentation graphique de la \autoref{fig:vis-vs-pre-2} correspond à la réduction transitive de la erlation de viisbilité.
Pour éviter toute ambiguité, les relations ne sont pas étiquettées.
...
L'exécution abstraite de la la \subref{fig:vis-vs-pre-2} correpond à la réduction transitive de l'exécution abstraite de la la \subref{fig:vis-vs-pre-1}.}\label{fig:vis-vs-pre}
\end{figure}
...
Pour faciliter la spécification du modèled e cohérence \ac{VFJC}, nous définissons dans la \autoref{def:known-malicious} l'enemble des pairs reconnus mal-intentionné en une opération.
La \autoref{tab:known-malicious} indique pour chaque opérations de la \autoref{fig:provably-malicious} les pairs qui sont reconnus mal-intentionnés.
...
Pour définir cette notion de direct nous introduisons une relation construite sur la relation de viisbilité~:
...
Il s'agit d'un comportement recherché car il renvoit à une sémantique usuelle pour ses utilisateur·ice·s~\autocite{preguia2018_crdt}.
...
Pour déterminer la composition de l'ensemble retournée par l'exécution de l'opération de lecture $a_5$, nous conservons uniquement l'opération avec l'horodatage le plus grand pour chaque valeur ajoutée au moins une fois dans l'ensemble.
...
&\trm{eval}_i(\tuple*{m, n}, \trm{rd}) \defeq \set*{v \given \tuple*{v, \_, \_} \in m}\\
...
L'état est définit comme un couple dont le premier élément contient l'ensemble des valeurs associées à leur identifiant unique, et le second élément est un nombre séquentiel utilisé pour générer les identifiants uniques.
