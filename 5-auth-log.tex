...
Pour rejoindre une collaboration, un nouveau pair doit d'abord se faire inviter par un pair déjà présent dans la collaboration.
Ce dernier, exécute une opération d'invitation qui génère l'identifiant $p_i$ du nouveau pair.
Il transmet cet identifiant et son journal au nouveau pair.
...
Le nouveau pair $p_i$ construit son journal $J$ et l'état de sa copie $S$ à partir du journal reçu.
Lors de la construction de son journal, il vérifie que le journal reçu a été correctement construit.
Il vérifie que chaque message est bien-formé et traite les messages dans l'ordre dans lequel ils ont été ajoutés dans le journal reçu.
...
Si le journal reçu est correctement construit, ses états successifs n'ont pas conduit à la présence de messages non-linéaires parmi ses messages maximaux.
Le nouveau pair n'a donc pas besoin d'exécuter d'opérations d'acquiescement lorsque la première condition est remplie.
Il doit simplement vérifier que l'ajout de messages dans son journal ne conduit pas à la présence de message non-linéaires parmi les messages maximaux de son journal.
...
Puisque le journal reçu est celui du pair qui invite le nouveau pair, l'invitation sera accepté immédiatement et dépend d'un préfixe du journal.
Les invités reconnus correspondent donc au invités connus du journal après l'ajout de l'invitation.
...
Pour les mêmes raisons que celles évoqués précédemment, il exécute une opération d'acquiescement avant l'acceptation immédiate d'un message lorsqu'il ne reconnaît pas encore l'auteur de ce message ou de l'une de ses dépendances en attente.

L'initialisation réussit si et seulement si le pair fait partie des invités connus du journal et l'ensemble des messages du journal reçu figurent dans le journal du nouveau pair (il n'y a pas de messages mal-formés et de message qui demeurent en attentes).
...
Si la condition n'est pas remplie l'initialisation est mise en échec.
Une mise en échec de l'initialisation survient lorsque le journal est mal-formé.
...
Ces dépendances non-linéaires ne son taps ajouté dans l'ensemble $\trm{max}_J$.
...
Les pairs essayent de tronquer leur journal pour réduire son occupation mémoire.
...
\paragraph{Déterminer les messages convergent-stables}
...
Par-ce-qu'il esr lui-même suspecté mal-intentionné par les autres pairs, déterminer les messages convergent-stables revient en quelque sortes à considérer tous les pairs, y compris lui-même, comme des pairs mal-intentionnés.
...
Un algorithme naïf pourrait parcourir pour chaque permutation chaque chemin de dépendance du graphe formé par le journal.
...
\begin{itemize}
    ...
    Par exemple, l'énumération $\tuple*{p_B, p_A, p_B}$ inclut toutes les permutations de l'ensemble $\tuple*{p_A, p_B}$.
    \item Au lieu de parcourir chaque chemin du graphe qui prend source au message dont la stabilité est testé, nous en parcourons un seul.
\end{itemize}

Un pair qui récupère un journal tronqué doit être capable de vérifier que le journal est correctement 
tronqué.
Si un pair détermine qu'un message est convergent-stable dans son journal, un autre pair qui récupère ce journal doit également la déterminer convergent-stable.
...
Pour assurer que l'énumération de pairs soit la même quelque soit le pair qui teste la stabilité d'un message, nous devons ordonner l'ensemble des pairs qui est passé à cette fonction.
...
L'un des articles~\autocite{adleman_1974_shortperm} présente une manière simple de construire une énumération de taille $N^2 -2N +4$ qui inclut l'ensemble des permutations de ces $N$ composantes.
L'\autoref{alg:all-perm-enum} est une adaptation de la construction mathématique décrite dans cet article.
...
Pour connaître les pairs reconnus mal-intentionnés d'un message $m$, nous devons construire le journal qui inclut $m$ et toutes ses dépendances.
Cette construction peut être coûteuse sauf pour les messages qui dépendent de tous les messages qui ont été précédemment ajoutés dans le journal.
C'est le cas des messages du possesseur du journal.
Cette optimisation est donc considérée uniquement lorsque la stabilité convergente d'un message du possesseur du journal est testée.
Lorsque le message n'est pas un message du possesseur du journal nous pouvons toutefois retirer les pairs reconnus mal-intentionnés dans l'état $I$.
Ce qui correspond à un sous-ensemble des pairs reconnus mal-intentionnés en $m$.

\begin{algorithm}[ht]
\caption{Détermine si un message $x$ est convergent stable dans $J$.}\label{alg:convergent-stable}
\begin{algorithmic}[1]
\Function{$\trm{convergentStable}_i$}{$x$}
\textbf{require} $x \in J$
    ...
    \State \textbf{let} $k = 1$
    \If{$\trm{peer}_x = p_i$}\Comment{l'auteur de $x$ est-il $p_i$, le possesseur du journal $J$~?}
        \State $Z \assign Z \lappend \tuple*{y_1, \dotsc, x}$\Comment{ajoute $x$ et ses dépendances à $Z$}
        \State $k \assign \trm{length}(\tuple*{y_1, \dotsc, x})$
    \EndIf
    \State \textbf{let} $\tuple*{p_1, \dotsc, p_n} \assign \trm{allPermEnum}(\trm{invited}_J \sminus \trm{knownMalicious}_Z)$
    \State \textbf{let} $j \assign 1$
    \State \textbf{let} $\trm{prev} \assign x$
    \While{$j \leq n \land k \leq \trm{length}(J)$}
        \If{$y_k = x \lor \trm{id}(prev) \in \trm{deps}_{y_k}$}\Comment{$y_k$ fait-il partie du chemin à parcourir~?}
...
Nous créons d'abord un journal temporaire $Z$ dont il initialise les structures de données dépendantes à l'aides des structures de données dépendantes de l'état $I$ (ligne 2 à 4).
Si le message est un message du possesseur du journal, alors $m$ et ses dépendances sont ajoutées à $Z$ (ligne 7 à 10).
Ces messages forment un préfixe du journal $J$.
Dans ce cas, $\trm{knownMalicious}_Z$ correspond également aux pairs reconnus mal-intentionnés en $m$.
Nous construisons ensuite l'énumération de pairs qui inclut les permutations des pairs invités auquel est retiré les pairs reconnus mal-intentionnés dans $Z$ (ligne 11).
Le reste de l'algorithme parcourt l'énumération de pairs et le journal.
Si l'énumération de pairs est entièrement parcouru, le message $m$ est convergent-stables.
Si le message fait partie du chemin à parcourir (ligne 15), nous avançons d'un pas dans l'énumération de pairs si et seulement si le pair actuel de l'énumération est \begin{inlinelist}\item l'auteur du message actuel, ou si il est \item le pair invité dans le message actuel (le message actuel est une invitation), ou si il est \item reconnu mal-intentionné dans $Z$ (il est présent dans l'ensemble $\trm{knwonMalicious}_Z$) \end{inlinelist}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{ccccccc}
        ...
        \toprule
        2 à 7 et 11 à 13 & $1$ & $p_M$ & $m_1$ & $1$ & $a_1$ & $\emptyset$ \\
        14, 15, 24, 25 & $1$ & $p_M$ & $m_1$ & $2$ & $a_2$ & $\emptyset$ \\
        14, 15, 24, 25 & $1$ & $p_M$ & $m_1$ & $3$ & $a_3$ & $\emptyset$ \\
        14, 15, 24, 25 & $1$ & $p_M$ & $m_1$ & $4$ & $m_1$ & $\emptyset$ \\
        14 à 17 & $2$ & $p_A$ & $m_1$ & $4$ & $m_1$ & $\emptyset$ \\
        14 à 16, 19 à 21 & $2$ & $p_A$ & $m_1$ & $5$ & $a_4$ & $\emptyset$ \\
        14 à 17 & $3$ & $p_B$ & $m_1$ & $5$ & $a_4$ & $\emptyset$ \\
        14 à 16, 19 à 21 & $3$ & $p_B$ & $a_4$ & $6$ & $m_1'$ & $\set*{p_M}$ \\
        14, 15, 24, 25 & $3$ & $p_B$ & $a_4$ & $7$ & $b_1$ & $\set*{p_M}$ \\
        14, 15, 24, 25 & $3$ & $p_B$ & $a_4$ & $8$ & $b_2$ & $\set*{p_M}$ \\
        14 à 17 & $4$ & $p_M$ & $a_4$ & $8$ & $b_2$ & $\set*{p_M}$ \\
        14 à 17 & $5$ & $p_A$ & $a_4$ & $8$ & $b_2$ & $\set*{p_M}$ \\
        14 à 16, 19 à 21 & $5$ & $p_A$ & $b_2$ & $9$ & $a_5$ & $\set*{p_M}$ \\
        14 à 17 & $6$ & $p_B$ & $b_2$ & $9$ & $a_5$ & $\set*{p_M}$ \\
        14 à 16, 19 à 21 & $6$ & $p_B$ & $a_5$ & $10$ & $b_3$ & $\set*{p_M}$ \\
        14 à 17 & $7$ & $p_M$ & $a_5$ & $10$ & $b_3$ & $\set*{p_M}$ \\
        14 à 17 & $8$ & & $a_5$ & $10$ & $b_3$ & $\set*{p_M}$ \\
    \end{tabular}
...
\paragraph{Tronquer le journal}
Pour éviter de tester la stabilité convergente de plusieurs messages et prendre avantage de l'optimisation qui concerne les messages du possesseur du journal, un pair teste la stabilité d'un seul message dont il est l'auteur.

Si le message est convergent-stable, alors ses dépendances le sont également.
Puisqu'il s'agit d'un message du possesseur du journal, ces messages convergent-stables forment un préfixe.
...
Le pair supprime donc un le préfixe $a_1 \dep a_2$.
...
Le message qui a été choisit pour stabiliser et tronquer le journal est conservé dans la variable $\trm{ref}_I$.
Les message supprimés du journal sont un à un intégré à l'état $I$.
...
\Procedure{$\trm{attemptTruncation}_i$}{}
    ...
    \If{$\trm{convergentStable}_i(y_k)$}
        \State $\trm{ref}_I \assign \tuple*{y_k, y_n}$ \Comment{mise à jour des références de stabilisation}
        \State \textbf{let} $j \assign 1$
...
\paragraph{Initier une collaboration.}
Pour initier une collaboration, un pair exécute une première opération qui l'invite lui-même.
Avant d'effectuer cette exécution, le pair initialise les structures de données utilisées par le protocole.
La copie $S$ du contenu partagé est initialisée à un état déterminé par le protocole de réplication.
Les autres structures de données sont initialisées comme suit.
...
\begin{align*}
    ...
    &\trm{vv}_J \assign \emptyset\\
    &\trm{knownMalicious}_J \assign \emptyset\\
    &\trm{ackInvited} \assign \emptyset\\
    &\trm{pending} \assign \emptyset\\
    &I \assign S\\
    &\trm{vv}_I \assign \emptyset\\
    &\trm{knownMalicious}_I \assign \emptyset\\
    &\trm{ref}_I \assign \varnothing\\
\end{align*}

\paragraph{Rejoindre une collaboration.}
Pour rejoindre une collaboration, un nouveau pair doit d'abord se faire inviter par un pair déjà présent dans la collaboration.
Ce dernier, exécute une opération d'invitation qui génère l'identifiant $p_i$ du nouveau pair.
Il transmet cet identifiant au nouveau pair, ainsi que son journal, son état $I$, les structures dépendantes de $I$ ($\trm{vv}_I$ et $\trm{knownMalicious}_I$), et les références de stabilisation $\trm{ref}_I$.
Pour le distinguer du journal du pair $p_i$ nous le notons $G$.
Nous n'avons pas besoin de distinguer son état $I$ de celui du pair $p_i$ étant donnée que $p_i$ utilise directement l'état $I$ qu'il reçoit.
Un pair mal-intentionné peut transmettre des données arbitraires.

Nous supposons que le journal reçu $G$ est tronqué.
Si il ne l'est pas le pair peut suivre les mêmes étapes que celles présentés dans le protocole à journaux complets.
...
\begin{align*}
    &J \assign \emptyset\\
    &\trm{max}_J \assign \emptyset\\
    &\trm{vv}_J \assign \trm{vv}_I\\
    &\trm{knownMalicious}_J \assign \trm{knownMalicious}_I\\
    &\trm{ackInvited} \assign \emptyset\\
    &\trm{pending} \assign \emptyset\\
    &S \assign I\\
\end{align*}

Il ajoute à $J$ les messages du journal $J_j$ que $p_j$ déclare comme convergent-stables.
Il s'agit des messages qui précédent ou sont égal à la première référence de stabilisation dans le journal $J_j$.
Pour les messages restants, le pair suit les même étapes que celles présentés pour le protocole à journaux complets.
Il demande à sa couche réseau de lui livrer les messages restants de $J_j$, dans l'ordre dans lequel ils ont été ajoutés dans $J_j$.
Il ne peut pas exécuter d'opérations d'acquiescement avant d'avoir ajouté à son journal le message qui l'invite.
Une fois ce message ajouté, il exécute une opération d'acquiescement seulement avant l'ajout d'un message dont l'auteur n'est pas un invité reconnu.
Pour chaque message qu'il ajoute dans son journal il vérifie qu'il n'y a pas de message non-linéaire dans les messages maximaux du journal.

\begin{equation*}
    x \in \trm{max}_J \implies \trm{peer}_x \not\in \trm{knownMalicious}_J
\end{equation*}

Il vérifie également que tous les messages qui précédent la première référence de stabilisation dans le journal sont des dépendances directes ou transitives de la première référence de stabilisation.

Il vérifie que les message non-stables ne déclarent pas une dépendance sur un message qui précéde la première référence de stabilisation.

Lorsqu'il ajoute la deuxième référence de stabilisation au sein de son journal, il vérifie que la première référence de stabilisation est bien un message convergent-stable du point de vue du pair $p_j$.

La livraison de son message d'invitation lui permet de s'assurer que l'état $I$ et ses structures de données dépendantes sont cohérents.
...
Inviter un pair le force donc à se reconnaître lui-même et à reconnaître les pairs invités dans les dépendances de son invitation.
...
\begin{algorithm}[ht]
\caption{Initialisation du journal et de la copie du pair $p_i$ à partir d'un journal tronqué $G$ et d'un état $I$ d'un pair $p_j$}\label{alg:pruned-log-init}
\begin{algorithmic}[1]
\Procedure{$\trm{init}_i$}{$G, p_j$}
    \State \textbf{let} $\tuple*{l,h} \assign \trm{ref}_I$
    \State \textbf{check} $1 <l < h < n$
    \State \textbf{let} $\tuple*{y_1, \dotsc, y_n} \assign G$
    \State \textbf{check} $\trm{peer}_{y_l} = p_j$
    ...
    \While{$k \leq n$}
        ...
        \Else
            \State $\trm{pending} \assign \trm{pending} \lappend \tuple*{y_k}$
        \EndIf
        \State \textbf{check} $k = h \implies \trm{convergentStable}_j(y_l)$
        \State $k \assign k + 1$
    \EndWhile
    \State \textbf{check} $\trm{pending} = \emptyset \land p_i \in \trm{invited}_J$
\EndProcedure
