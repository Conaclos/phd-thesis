...
\begin{tikzpicture}
    ...
    \path node (a1) {$a_1$}
        ...
        to +(6*\hsep,0) node (b2) {$b_2$}
        to +(7*\hsep,0) node (b3) {$b_3$}
    ;
    % Dependency
    \foreach \src/\dest in {a1/a2,a2/b1,b1/m1,m1/a3,m1p/b2,b2/b3}
        \draw[pre] (\src) to (\dest);
    \draw[vis,bend right=25] (b1) to (m1p);
    \draw[vis,bend left=25] (a3) to (b3);
\end{tikzpicture}
...
\begin{table}[ht]
    ...
    \begin{tabular}{ccccc}
        ...
        $m_1$ & $\tuple*{\trm{add}, 1}$ & $p_M$ & $1$ & $\set*{b_1}$ \\
        $a_3$ & $\tuple*{\trm{ack}}$ & $p_A$ & $3$ & $\set*{m_1}$ \\
        $m_1'$ & $\tuple*{\trm{add}, 2}$ & $p_M$ & $1$ & $\set*{b_1}$ \\
        $b_2$ & $\tuple*{\trm{add}, 3}$ & $p_B$ & $2$ & $\set*{m_1'}$ \\
        $b_3$ & $\tuple*{\trm{add}, 4}$ & $p_B$ & $3$ & $\set*{a_3,b_2}$ \\
    \end{tabular}
    \caption{Contenu et des méta-données des messages de la \autoref{fig:log-example-2}}\label{tab:msg-metadata}
\end{table}
...
\begin{table}[ht]
    ...
    \begin{tabular}{cccccc}
        ...
        $a_3$ & $\set*{1}$ & $\set*{a_3}$ & $\emptyset$ \\
        $m_1'$ & $\set*{1,2}$ & $\set*{a_3,m_1'}$ & $\set*{p_M}$ \\
        $b_2$ & $\set*{1,2,3}$ & $\set*{a_3,b_2}$ & $\set*{p_M}$ \\
    ...
    \begin{tabular}{ccc}
        ...
        $m_1$ & $\set*{\tuple*{p_A, 2}, \tuple*{p_B, 1}, \tuple*{p_M, 1}}$ & $\set*{p_A,p_B,p_M}$ \\
        $a_3$ & $\set*{\tuple*{p_A, 3}, \tuple*{p_B, 1}, \tuple*{p_M, 1}}$ & $\set*{p_A,p_B,p_M}$ \\
        $m_1'$ & $\set*{\tuple*{p_A, 3}, \tuple*{p_B, 1}}$ & $\set*{p_A,p_B,p_M}$ \\
        $b_2$ & $\set*{\tuple*{p_A, 3}, \tuple*{p_B, 2}}$ & $\set*{p_A,p_B,p_M}$ \\
        $b_3$ & $\set*{\tuple*{p_A, 3}, \tuple*{p_B, 3}}$ & $\set*{p_A,p_B,p_M}$ \\
    \end{tabular}
    \end{subtable}
...
Pour simplifier, le protocole peut exécuter systématiquement uen opération d'acquiescement si un message est ajouté avec des dépendances en attente.
