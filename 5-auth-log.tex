...
Le rejet des chaînes d'observation cumulée d'une énumération $P$ de pairs implique le rejet des chaînes d'observation cumulée des énumérations de pair qui préfixent $P$.
EXEMPLE.
...
Soit une énumération de pairs $P \defeq \tuple*{p_1, \dotsc ,p_n}$ tel que $p_n$ tel que $p_1, \dotsc, p_{n-1}$ sont des pairs mal-intentionnés, et $p_n$ est un pair honnête.
$A'$ ne peut contenir une chaîne d'observation cumulée $Z$ de $P$ dont la première opération est concurrente à $x_0$ si et seulement si $A$ contient une chaîne de visibilité $X \defeq x_1 \viseq \dotsb \viseq x_n$ tel que $x_0 \viseq x_1$ et le $i$-ème pair de $P$ est lié à la $i$-ème opération de $V$ par l'une des relations suivantes~: \begin{inlinelist}
...
\begin{proof}
La proposition est décomposable en deux implications.
Nous démontrons dans un premier temps que l'existence de $X$ empêche l'existence d'une chaîne d'observation cumulée $Z \defeq z_1 \idr \dotsb \idr z_m$ de $P$.
Dans un deuxième temps nous démontrons que $Z$ ne peut exister seulement si $X$ n'existe pas.

%\textbf{Première démonstration.}
Procédons par contradiction et récurrence.
Supposons que les châines $X$ et $Z$ existent.
$x_0$ est concurrente à $z_1$.
D'après la \autoref{th:concur-triad}, $z_1$ est concurrente à $x_1$.
$z_1$ et $x_1$ sont exécutées par le même pair, elles sont donc non-linéaires.
Montrons que si pour tout $j < k$ avec $1 < k \leq m$, $z_j$ est non-linéaire avec une opération de $X$, alors $z_k$ est non-linéaire avec une opération $x_l$ de $X$.
$x_l$ retourne-avant $z_k$.
$x_l$ est donc concurrente ou visible à $z_k$.
...
$z_1$ est non-linéaire à $x_1$.
Par récurrence nous en déduisons qu'au moins une opération de $Z$ est non-linéaire à $x_n$.
$p_n$ est donc un pair mal-intentionné.
Ce qui contredit une hypothèse initiale.

%\textbf{Deuxième démonstration.}
%Supposons que les châines $X$ et $Z$ existent.
