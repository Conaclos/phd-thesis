...
\end{subfigure}
\caption[Exemple d'opérations causalement stables]{Un ensemble de trois pairs $\trm{Peers} = \set{p_A, p_B, p_C}$ modifie un contenu répliqué.
...
Lorsqu'un pair est invité dans un embranchement rejeté par les autres pairs et qu'il contribue sur cet embranchement, il est exclu de manière permanente par les autres pairs.
...
\emph{SnapDoc} a pour objectif de masquer l'historique de la collaboration aux pairs qui rejoignent la collaboration.
Les pairs maintiennent un journal infalsifiable des messages qu'ils échangent.

Un pair ne transmet pas son journal.
Il transmet à un nouveau pair l'état de sa copie du contenu partagé, le dernier message de chaque pair connu, et une preuve de l'absence de messages non-linéaires (et donc d'embranchements) au sein de son journal.

\emph{SnapDoc} suppose l'utilisation du type de données répliquées \emph{TreeDoc}\footnote{\emph{TreeDoc} est présenté dans le \autoref{ch:dotted-logootsplit}} pour le contenu répliqué.
L'état de la copie est décomposable en éléments irréductibles.
Chaque message contient un accumulateur cryptographique qui inclut l'ensemble des éléments irréductibles de l'état au moment de la génération du message.
À la récéption d'un message, un pair vérifie que cet accumulateur est correctement calculé.
Un pair transmet à un nouveau pair le dernier message de chaque pair.
Le nouveau pair vérifie à l'aide des accumulateurs de ces messages, que les éléments irréductibles observés par les pairs sont authentiques.

Un pair maintient pour chaque pair qu'il connaît (y compris lui-même) un arbre de \emph{Merkle} dans lequel il enregistre leurs messages.
Un message contient un \emph{hash} $mh$ qui couvre l'ensemble des arbres de \emph{Merkle} maintenus par l'auteur du message.
Un nouveau pair reçoit une preuve que le \emph{hash} $mh$ de chaque message qu'il a reçu couvre bien un préfixe des arbres du pair qui l'invite.
Il peut donc s'assurer que l'historique de chaque pair est un préfixe de l'historique du pair qui l'invite.
Ce qui démontre l'absence d'embranchements dans le journal.
...
Contrairement à notre protocole à journaux tronqués, \emph{SnapDoc} ne permet pas aux pairs de tronquer leur journal.
