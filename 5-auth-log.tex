...
Malheureusement chaque pair doit conserver l'intégralité de son journal pour \begin{inlinelist}
...
\subref{fig:invite-by-log-2} Carol vérifie l'authenticité du journal et construit le dessin à partir du journal.}
\label{fig:invite-by-log}
\end{figure}

Un journal enregistre l'ensemble des modifications qui ont été jouées sur un document partagé.
...
\centering
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:invite-by-snaplog-2}
\end{subfigure}
\caption[Transmission d'un journal tronqué et d'une instantané à une nouvelle collaboratrice]{Alice et Bea répliquent de manière optimiste un dessin.
Ils utilisent un journal authentifié pour assurer la convergence des copies des pairs honnêtes.
Carol veut rejoindre la collaboration.
\subref{fig:invite-by-log-1} Alice envoie une instantané de l'état actuel du dessin, ainsi qu'une version tronqué de son journal authentifié.
\subref{fig:invite-by-log-2} Carol vérifie l'authenticité du journal tronqué.
Elle vérifié également l'authenticité de l'instantané à partir du journal tronqué.}
\label{fig:invite-by-snaplog}
\end{figure}
...
%Nous adoptons donc un point de vue subjectif.
...
%Dans le~\autoref{ch:background} nous avons vu que nous pouvions organiser les modèles de cohérence en hiérarchie.
%Un modèle de cohérence $C$ est plus fort qu'un modèle de cohérence $C'$ si il existe au moins une exécution abstraite qui respecte $C'$ et ne respecte pas $C$.
...
\begin{definition}[Opération stabilisable]
...
\end{align*}
\end{definition}
% TODO: vérifier si on peut remplacer \sqsupset_C par \sqsupseteq_C

Nous recherchons un modèle de cohérence stabilisable qui est réaliste dans le contexte de notre travail~: il doit garantir la disponibilité en lecture et en écritures des copies, ainsi que la convergence à terme des copies.
...
\end{tikzpicture}
\caption{}
\label{fig:vfjcs-one-malicious-1}
\end{subfigure}%
...
\end{tikzpicture}
\caption{}
\label{fig:vfjcs-one-malicious-2}
\end{subfigure}
\caption[Stabilité \acl{VFJC} en présence d'un pair mal-intentionné]{Exemple d'exécutions abstraites avec des opérations vfjc-stables (vfjcs) en présence d'un seul pair mal-intentionné $p_M$ et de deux pairs honnêtes $p_A$ et $p_B$.}
\label{fig:vfjcs-one-malicious}
\end{figure}

En présence de plusieurs pairs mal-intentionnés, il faut également s'assurer que chaque pair mal-intentionné ne peut pas accepter une opération concurrente à $x$ d'un autre pair mal-intentionné.
...
Au cours du temps, les pairs joignent et quittent la collaboration.
...
La \autoref{def:invitation-consistency} formalise le modèle de cohérence \emph{Invitation}. La \autoref{def:dyn-causal-consistency} présente le modèle de cohérence causale dynamique qui correspond à la conjonction du modèle de cohérence causale et du modèle de cohérence \emph{Invitation}.
...
\begin{subfigure}{\linewidth}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:dyn-causal-example-1}
\end{subfigure}
\par\medskip
\begin{subfigure}{\linewidth}
    \centering
...
\begin{definition}[Observateurs honnêtes requis]
...
\begin{definition}[Invités connus]\label{def:known-invited}
...
\begin{definition}[Observateurs d'une invitation]\label{def:invitation-obs}
...
Lorsqu'un pair honnête est invité dans un embranchement rejeté par les autres pairs et qu'il contribue sur cet embranchement, il est de manière permanente, exclut par les autres pairs honnêtes.
...
L'exécution d'une opération d'interrogation est simplement exécutée sur le contenu de type $T$.
...
Un pair honnête produit une séquence de messages avec des nombres séquentiels contiguës qui débutent à $1$.
...
\paragraph{Instantané authentifiée} Au moment de la rédaction de ce manuscrit, \textcite{kollmann2019_snapdoc} ont présenté SnapDoc, un protocole qui vise des objectifs similaires aux nôtres.
