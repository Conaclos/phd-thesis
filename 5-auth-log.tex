...
Dans le \autoref{ch:problematic} nous avons vu que la réplication optimiste d'un contenu et la modification concurrente de ce contenu conduit à la divergence des copies du contenu.
...
Pour détecter les équivoques et intégrer à leur copie l'ensemble des modifications effectuées sur le contenu, les pairs peuvent maintenir des journaux authentifiés.
...
Ce coût est particulièrement notable lorsqu'un pair rejoint la collaboration et qu'il a besoin de l'ensemble du journal pour commencer à contribuer.
...
\end{subfigure}
\caption[Transmission d'un journal tronqué et d'un instantané à une nouvelle collaboratrice]{Alice et Bea répliquent de manière optimiste un dessin.
Ils utilisent un journal authentifié pour assurer la convergence des copies des pairs honnêtes.
Carol veut rejoindre la collaboration.
\subref{fig:invite-by-log-1} Alice envoie un instantané de l'état actuel du dessin, ainsi qu'une version tronquée de son journal authentifié.
\subref{fig:invite-by-log-2} Carol vérifie l'authenticité du journal tronqué.
Elle vérifie également l'authenticité de l'instantané à partir du journal tronqué.}\label{fig:invite-by-snaplog}
\end{figure}
...
Une exécution abstraite est suffisamment général pour représenter une exécution ou un journal.
...
Nous disons que le pair $p$ est un observateur connu en $z$ de l'invitation d'un pair $q$ si et seulement si $p$ a exécuté une opération tel que l'invitation de $q$ lui est visible et elle est visible à $z$.
...
\begin{equation*}\begin{split}
    ...
    &\set*{p \in \trm{Peers} \given \exists x, y \in A \qsep x \viseq y \viseq z \land \trm{call}(x) = \trm{invite}(q) \land \trm{peer}(y) = p}
\end{split}\end{equation*}
\end{definition}
