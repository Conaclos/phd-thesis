...
\paragraph{Troncature du journal.}\index{troncature-journal@Troncature du journal} Au lieu de préserver l'intégralité du journal, seules les modifications nécessaires à la détection d'équivoques pourraient être conservées.
...
\paragraph{Transmission d'un état authentifiable.}\index{authentification-etat@Authentification d'un état} Au lieu de récupérer un journal dans son intégralité, un pair qui rejoint une collaboration pourrait récupérer l'état d'une copie du contenu partagé.
...
% Des pairs malintentionnés peuvent trouver un intérêt à compromettre la convergence des copies des pairs honnêtes.
...
% En effet, un pair malintentionné peut facilement falsifier un état du document avant de la transmettre à un nouveau pair.
...
% Par exemple, dans la \autoref{fig:log-example}, si nous savons que toutes les opérations futures dépendent de l'opération $a_4$ et que $a_4$ est suffisante à la vérification future de la cohérence du journal, alors les opérations qui précédent $a_4$ peuvent être supprimées du journal.
...
% Dans la \autoref{sec:stability} nous développons le concept de stabilité et nous recherchons un modèle de cohérence stabilisable en présence de pairs malintentionnés.
...
%
%Les dépendances déclarées par les pair malintentionnées ne traduisent pas nécessairement leurs observations réelles.
...
%    \end{dcases}
%\end{align*}
%\item L'histoire $H$ n'inclut pas d'opérations d'interrogation.
%\begin{equation*}
...
%
%\begin{proof}
%Suivant l'\autoref{th:rb-linear-order}, nous déduisons que toute restriction de la relation \emph{retourne-avant} aux opérations d'un pair donné (malintentionné ou honnête) est une énumération.
...
%Par conséquent, toutes les opération, y compris les opérations émises par des pairs malintentionnées, sont observées par au moins un pair honnête.
...
%Nous savions alors quels pairs sont honnêtes et lesquels sont malintentionnés.
...
%Le pair ne sait pas si un pair donné est honnête ou malintentionné.
%Le pair pourrait faire confiance à un ensembles de pairs qu'il considère honnête.
%Nous préférons prendre une approche plus pessimiste : un pair honnête considère que tout autre pair est potentiellement malintentionné.
%
%\begin{remark}
%Dans une exécution abstraite associée à un journal, tous les pairs, à l'exception du possesseur du journal, sont considérés malintentionnés.
...
% Nous recherchons un modèle de cohérence qui puisse être respecté par un protocole de réplication optimiste qui tolère la présence de pairs malintentionnés et permet la stabilisation d'opérations.
...
Pour définir la stabilité au sein d'une exécution abstraite, la \autoref{def:abstract-exec-extension} introduit le concept d'extension d'une exécution abstraite\index{extension-execution-abstraite@Extension d'une exécution abstraite}.
...
Dans une exécution abstraite, une opération est stable\index{operation-stable@Opération stable} une fois qu'elle est visible à toute opération future.
...
\begin{theorem}[Force des modèles de cohérence et stabilité]\label{th:stability-hierarchy}
...
La \autoref{def:stabilisable}\index{operation-stabilisable@Opération stabilisable} exprime cette attente sous la forme d'une propriété de vivacité.
...
\index{stabilite-causale@Stabilité causale}
...
$b_1$ et $c_2$ sont respectivement visibles à toute opération future de $p_B$ et $p_C$.
...
\begin{theorem}[Modèle de cohérence causale stabilisable]\label{th:stabilizable-causal}
...
\index{stabilite-vfjc@Stabilité \acl{VFJC}}
...
\paragraph{En présence d'un pair malintentionné $p_M$,}
...
% Si l'auteur de $y$ est le pair malintentionné, alors elle est visible à au moins une opération $z$ d'un pair honnête.
% Si $x$ est stable, alors pour toute extension de $A$ il n'existe pas une opération $y$ d'un pair honnête concurrente à $x$ et il n'existe pas une chaîne de visibilité immédiate $y \idr z$ avec $y$ tel que l'auteur de $y$ est le pair malintentionné et l'auteur de $z$ est un pair honnête.
%
% Une opération $x$ devient stable lorsqu'il n'est plus possible d'exécuter une opération qui lui est concurrente.
% En présence d'un seul pair malintentionné $p_M$, nous déduisons qu'une opération $x$ devient stable lorsque les pairs honnêtes observent $x$ et qu'ils ne peuvent plus accepter d'opérations concurrentes à $x$ de $p_M$.
...
\paragraph{En présence de plusieurs pairs malintentionnés.}
...
\end{inlinelist}.
% La deuxième condition implique l'inexistence d'une chaîne de visibilité immédiate composée d'opérations de pairs malintentionnés et d'un pair honnête.
...
Chaque sous-ensemble est associé à une énumération de pairs malintentionnés $P \defeq \tuple*{p_1, \dotsc, p_n}$ et est composé des chaînes d'observations cumulées de $P$.\index{@chaine-observations-cumuleeChaîne d'observations cumulées}
...
% L'ensemble des châines d'observations cumulées de $P$ contient l'ensemble des chaînes d'observations cumulées des préfixes de $P$.
% Par exemple, l'ensemble des chaînes d'observations cumulées de $\tuple*{p_G, p_M}$ inclut l'ensemble des chaînes d'observations cumulées de $\tuple*{p_G}$.
% Si un pair rejette les châines d'observations cumulées de $P$ alors il rejette les châines d'observations cumulées des préfixes de $P$.
% Pour rejeter l'ensemble des chaînes d'observations cumulées d'énumération de pairs malintentionnés, nous pouvons donc exclure l'ensemble des chaînes d'observations cumulées des permutations des pairs malintentionnés.
% Pour un ensemble de deux pairs malintentionnés $p_G$ et $p_M$, nous devrions donc rejeter les chaînes d'observations cumulées de $\tuple*{p_G, p_M}$ et $\tuple*{p_M, p_G}$.

\begin{definition}[Chaîne d'observations cumulées]\label{def:cumulated-obs}
Soit une exécution abstraite $A$ qui respecte le modèle de cohérence \acs{FJC}.
Une chaîne de visibilité immédiate $Z \defeq z_1 \idr \dotsb$ de $A$ est une chaîne d'observations cumulées d'une énumération de pairs distincts $\tuple*{p_1, \dotsc, p_n}$ si et seulement si elle inclut une chaîne de visibilité $Y \defeq y_1 \vis \dotsb \vis y_n$ telle que l'auteur de la $i$-ième opération de $Y$ est le $i$-ième pair de l'énumération, et l'auteur de toute opération de $Z$ est l'auteur d'une opération de $Y$ qui lui est visible ou égale.
On a $y_1 = z_1$.
\begin{equation*}
    \forall i \in \irange{1}{n} \qsep p_i = \trm{peer}(y_i) \land \forall z \in Z \:\exists y \in Y \qsep y \viseq z \land \trm{peer}(y) = \trm{peer}(z)
\end{equation*}
\end{definition}
...
\begin{tikzpicture}
    ...
    \foreach \src/\dest in {z1/r1,r1.east/zn,zn/r2.west,r2.east/zm,zm/r3.west}
        \draw[pre] (\src) to (\dest);
\end{tikzpicture}
\caption[Ensemble des chaînes d'observations cumulées d'une énumération de pair]{Ensemble des chaînes d'observations cumulées de l'énumération de pairs $\tuple*{p_G,p_M,p_O}$.}\label{fig:cumulated-obs-3peers}
\end{figure}

La \autoref{th:cumulated-obs-exclusion} présente les conditions à remplir pour qu'un pair honnête rejette les chaînes d'observations cumulées d'une énumération de pairs malintentionnés dont la première opération est concurrente à une opération $x$.
...
\end{subfigure}
\caption{Exécutions abstraites avec des opérations vfjc-stables (vfjcs) en présence d'un pair honnête $p_A$, et de deux pairs malintentionnés $p_M$ et $p_G$.}\label{fig:vfjcs-two-malicious}
\end{figure}

Le rejet des chaînes d'observations cumulées d'une énumération $P$ de pairs implique le rejet des chaînes d'observations cumulées des énumérations de pairs qui préfixent $P$.
...
%         \draw[pre] (\src) to (\dest);
% \end{tikzpicture}
% \caption{Exemple d'exécutions abstraites avec des opérations vfjc-stables (vfjcs) en présence de deux pairs malintentionnés $p_M$ et $p_G$ et d'un pair honnête $p_A$.
% Le pair $p_M$ est reconnu malintentionné par $p_A$ et $p_G$.}\label{fig:vfjcs-evicted-peer}
% \end{figure}
...
%\begin{definition}[Pairs malintentionnés évincés]\label{def:evicted-peers}
%Dans une exécution abstraite $A$ qui respecte le modèle de cohérence \ac{VFJC}, un pair malintentionné est évincé de $A$ si et seulement si il n'existe pas une extension qui respecte le modèle de cohérence \ac{VFJC} et qui inclut une opération de ce pair absente de $A$.
%$\trm{Evicted}_A$ et $\overline{\trm{Evicted}}_A$ dénotent respectivement l'ensemble des pairs malintentionnés évincés et l'ensemble des pairs malintentionnés non-évincés.
%\begin{align*}
...
\begin{theorem}[Modèle de cohérence VFJC stabilisable]\label{th:stabilizable-vfjc}
...
\index{stabilite-causale-dyn@Stabilité causale dynamique}
...
Nous introduisons une opération d'invitation\index{operation-invitation@Opération d'invitation} pour suivre l'évolution de la composition du groupe.
...
La \autoref{def:invitation-consistency} regroupe ces contraintes au sein du modèle de cohérence \emph{Invitation}\index{coherence-invitation@Cohérence Invitation}.
...
\begin{definition}[Modèle de cohérence Invitation]\label{def:invitation-consistency}
...
Nous parlons d'\emph{observateurs requis}\index{obseravteur-honnete-requis@Observateurs honnêtes requis}.
...
\begin{theorem}[Modèle de cohérence causale dynamique stabilisable]\label{th:stabilizable-dyn-causal}
...
\index{stabilite-vfjc-dyn@Stabilité~\acs{VFJC} dynamique}
...
Le \autoref{tab:invited} indique les observateurs connus de l'invitation\index{observateurs-connus-invitation@Observateurs connus d'une invitation} du pair $p_G$ en chaque opération de l'exécution abstraite de la \autoref{fig:vfjc-invitation-issue}.
...
\begin{definition}[Cohérence \acl{VFJC} dynamique]\label{def:svfjc-consistency}
...
\begin{description}
  ...
  %L'auteur d'une opération $x$ qui est immédiatement visible à une opération $z$ doit être connu comme invité dans chaque opération $y$ qui est immédiatement visible à $z$.
  %\begin{equation*}
...
Lorsqu'un pair est invité dans un embranchement rejeté par les autres pairs, il est exclu de manière permanente par les autres pairs.
...
\begin{proposition}\label{th:cumulated-obs-exclusion-dyn}
...
Le rejet des chaînes d'observations cumulées d'une énumération $P$ de pairs n'implique pas forcément le rejet des chaînes d'observations cumulées des énumérations de pairs qui préfixent $P$.
...
% et $x_k$ est visible ou égale à une opération $y$ qui invite $p_n$ ou est exécutée par $p_n$.
\begin{equation*}\begin{split}
\MoveEqLeft\trm{stable}_A^{\trm{DynVFJC}}(x_0) \iff\\
    ...
    %&\exists y \in A \qsep x_k \viseq y \land (\trm{peer}(x_k) = p_n \lor \trm{call}(y) = \trm{invite}(p_n)) \:\land\\
...
Prenons pour exemple l'exécution abstraite $A$ de la \autoref{fig:dvfjcs-example}.
...
% En principe un pair malintentionné peut exécuter une seconde opération d'invitation initiale.
...
\begin{theorem}[Modèle de cohérence VFJC dynamique stabilisable]\label{th:stabilizable-dyn-vfjc}
...
%
%Notre contexte exige de prendre en compte la présence de pairs malintentionnées dans le groupe de pair, ainsi que l'évolution de la composition du groupe.
...
%\item les pairs reconnus malintentionnés en $x$ $\trm{knownMalicious}(x)$
%\end{itemize}
%
%\begin{definition}[Journal augmenté bien-formé]
%Un journal (augmenté) $J$ est bien-formé si et seulement si~:
%\begin{description}
%\item[pairs reconnus malintentionnés]
%\begin{equation*}
...
Les protocoles que nous décrivons nécessitent la présence d'une \emph{opération d'invitation} et d'une \emph{opération d'acquiescement}\index{operation-acquiescement@Opération d'acquiescement}.
...
Pour former un journal infalsifiable\index{journal-infalsifiable@Journal infalsifiable}, chaque message est accompagné d'une signature cryptographique et référence éventuellement d'autres messages.
Les messages qu'il référence sont ses \emph{dépendances déclarées}\index{dependance-declarees@Dépendance déclarée}.
...
Un pair honnête produit une séquence de messages avec des numéros contigus\index{numero-message@Numéro d'un message}.
...
L'identifiant d'un pair est un couple constitué d'une clef publique et d'un \emph{hash}.\index{identifiant-pair@Identifiant d'un pair}
...
\end{tikzpicture}
\caption[Journal d'un pair]{Exemple du journal d'un pair.}\label{fig:log-example-2}
\end{figure}
%
\begin{table}[ht]
    \centering
    ...
    \end{tabular}
    \caption{Contenu et méta-données des messages du journal de la \autoref{fig:log-example-2}}
\end{table}
...
\index{structure-donnees-dependantes@Structures de données dépendantes}
...
Nous notons $\trm{vv}_J$ un vecteur de versions~\autocite{parker_1983_versionvector,mattern_1988_timevector}\index{vecteur-versions@Vecteurs de versions} qui associe à un pair le numéro du dernier message dont il est l'auteur qui a été ajouté dans $J$.
...
Un message bien-formé $m$ est non-linéaire à un message d'un journal $J$ si et seulement si il créée un embranchement ou si son auteur est déjà reconnu malintentionné dans $J$.
% Un message bien-formé ne peut pas joindre des embranchements dont son auteur est à l'origine.
% $m$ fait donc partie d'un embranchement.
% Il est non-linéaire à chacun et est non-linéaire avec les messages des autres embranchements.
%
...
Le pair déclare les messages maximaux du journal comme dépendances de son message.
...
Un pair vérifie d'abord que le message est bien-formé.\index{message-bien-forme@Message bien-formé}
...
%Le pair effectue cette vérification seulement lorsque l'auteur de $m$ ou de l'une de ses dépendances est un pair reconnu malintentionné.
%
%\begin{equation*}\begin{split}
...
%         \trm{ctx}_{\trm{pending}}(m) \lappend \set*{m} & \when \trm{accepted}_J(m)\\
%         \emptyset
%     \end{dcases}
% \end{equation*}
...
%         \emptyset & \when \trm{accepted}_J(m)\\
%         \set*{m}
%     \end{dcases} \sminus \begin{dcases}
%         \trm{ctx}_{\trm{pending}}(m) & \when \trm{accepted}_J(m)\\
%         \emptyset
%     \end{dcases}
% \end{equation*}
...
\end{tikzpicture}
\caption[Exécution du protocole à journaux complets par un pair $p_A$]{Exemple d'exécution du protocole à journaux complets par un pair $p_A$.
...
\begin{tikzpicture}[every node/.style={}]
    ...
    ;
\end{tikzpicture}
\caption[Exécution du protocole à journaux complets par un pair $p_B$]{Exemple d'exécution du protocole à journaux complets par un pair $p_B$.
...
La \autoref{def:exec-abs-log} définit l'exécution abstraite associée à un journal\index{execution-abstraite-associéé@Exécution abstraite associée à un journal}.
...
\end{tikzpicture}
\caption[Exécution abstraite associée à un journal]{Exécution abstraite associée au journal de la \autoref{fig:log-example-2}.}\label{fig:exec-abs-log-example}
\end{figure}

\begin{theorem}[Exécution abstraite associée bien-formée]\label{th:wf-abs-exec-log}
...
\begin{theorem}[Exécution abstraite associée DynVFJC]\label{th:abs-exec-full-log-consistency}
...
La \autoref{def:stable-msg} définit ce qu'est un message stable\index{message-stable@Message stable}, et le \autoref{th:op-msg-stable} créé une correspondance entre messages stables et opérations stables.
...
Deux journaux avec le même ensemble de messages ont le même ensemble de \emph{messages convergents-stables}\index{message-convergent-stable@Message convergent-stable}.
...
Le protocole présenté permet de produire des journaux dont les messages deviennent à terme stables.
...
% Un type de données répliquées inclut une garantie de convergence forte au sein de sa spécification.
% Sa spécification détermine la valeur de retour de toute opération de lecture en fonction des opérations de modification qui lui sont visibles.
% Certaines relations de visibilité entre des opérations de modification et des opérations de lecture peuvent avoir aucun effet sur la valeur de retour des opérations de lecture.
% La spécification ne requiert donc pas leur existence.
% Pour vérifier que notre protocole garantit la convergence des copies, nous employons un type de données répliquées dont chaque opération de modification a un effet notable sur la valeur de retour des opérations de lecture auxquelles il est visible.
% Nous employons un ensemble d'entiers répliqués munit d'une opération d'ajout $\trm{add}$ et d'une opération qui renvoie simplement les entiers présents dans l'ensemble.
% Chaque entier ajouté dans l'ensemble est unique.
% Cette contrainte permet de capturer avec précision les modifications prises en compte par une lecture.
% Les équations suivantes donnent la signature syntaxique et la spécification de ce type de données répliquées.
...
Nous parlons d'\emph{empreinte} d'un état $I$\index{empreinte-etat@Empreinte d'un état}, le \emph{hash} calculé sur $I$ et ses structures de données dépendantes.
...
\begin{algorithm}[ht]
\caption[Détermine si un message est convergent-stable]{Détermine si un message $m$ est convergent-stable dans $J$.}\label{alg:convergent-stable}
\begin{algorithmic}[1]
...
% Si ce n'est pas le cas, le nouveau pair est considéré malintentionné.
...
L'utilisation d'accumulateurs cryptographiques a un surcoût important qui rend le protocole inadapté à la réplication de contenus composés d'un nombre important d'éléments irréductibles.
...
Du point de vue de l'utilisabilité, \emph{SnapDoc} ne supporte pas tous les type de données répliquées.
