...
\item l'ajout de messages dans le journal conduit à la présence d'une opération non-linéaire parmi les messages maximaux du journal, ou si
...
Il peut simplement vérifier qu'à l'issue de l'ajout de messages dans le journal, il n'y a pas de messages d'un pair reconnu mal-intentionné dans les messages maximaux du journal.
...
\Procedure{$\trm{attemptTruncation}_i$}{}
    \State \textbf{let} $\tuple*{m_1, \dotsc} \assign J$
    \State \textbf{pick} $k \in \mathbb{N}^* \land \trm{peer}_{m_k} = p_i$
    \If{$\trm{convergentStable}_i(m_k)$}
        \State $\trm{ref}_I \assign m_k$ \Comment{$m_k$ est la nouvelle référence de troncature}
        \State \textbf{let} $j \assign 1$
        \While{$j < k \land \nexists l > k \qsep \trm{id}(m_j) \in \trm{deps}_{m_l} $}
            \State $J \assign J \sminus \tuple*{m_j}$ \Comment{supprime $m_j$ du journal $J$}
            \State $I \assign \trm{integrate}_i(I, m_j)$ \Comment{intégre $m_j$ dans l'état $I$}
            \State $j \assign j + 1$ \Comment{passe au message suivant}
        \EndWhile
    \EndIf
\EndProcedure
\end{algorithmic}
\caption{Algorithme qui essaye de tronquer le journal d'un pair $p_i$. L'algorithme met à journal le journal $J$, l'état $I$, ainsi que la réference de troncature $\trm{ref}_I$}
\end{figure}
...
Un message bien-formé pour le protocole à journaux tronqués est également bien-formé pour le protocole à journaux tronqués.
Les journaux maintenus par le protocole à journaux tronqués sont donc associées à des exécutions abstraites qui respectent le modèle de cohérence \emph{DynVFJC}.
...
Lorsqu'il détermine la stabilité d'un message, il se considère donc être le seul pair honnête.
...
Un message $m$ est \emph{DynVFJC} convergent-stable au sein d'un journal $J$ si et seulement si pour chaque permutations $\tuple{p_1, \dotsc, p_N}$ de l'ensemble des pairs invités $\trm{invited}_J$, il existe une chaîne de dépendance $m_1 \depeq \dotsb \depeq m_n$ tel que $m_1$ dépend de $m$ ou est égal à $m$, et le $j$-ème pair de la permutation est lié au $j$-ème message de la chaîne par l'une des relations suivantes~: \begin{inlinelist}
...
Il indique pour chaque permutation des pairs invités une chaîne de dépendance correspondante.
Par exemple, si nous prenons la permutation $\tuple*{p_M, p_B, p_A}$.
La châines de dépendances $b_2 \depeq b_2 \depeq a_5$ est valide.
...
Une implémentation naïve pourrait parcourir pour chaque permutation chaque chemin de dépendance du graphe formé par le journal.
...
Cet algorithme a une complexité linéaire par rapport aux nombre de pairs invités et du nombre de messages enregistrés dans le journal.
...
Puisque chaque pair doit être capable de vérifier que le journal d'un autre pair est correctement tronqué, ils doivent considérer pour un même journal, la même énumération de pairs et le même chemin à parcourir.

Pour ce faire, les identifiants des pairs sont totalement ordonnées.
l'ensemble des pairs invités $\trm{invited}_J$ est donc un ensemble ordonné.
...
La formation d'une énumération minimale qui inclut toutes les permutations est un problème ouvert.
Toutefois plusieurs articles~\autocite{engen2018_containing} prouvent des bornes supérieures sur la taille de cette énumération.
...
Nous proposons dans la \autoref{fig:all-perm-enum}, un algorithme qui traduit la construction mathématique de l'article.
...
\EndFunction
\end{algorithmic}
\caption{Algorithme de génération d'une énumération qui contient toutes les permutations d'un ensemble ordonné de pairs $\tuple*{p_1, \dotsc, p_n}$.}\label{fig:all-perm-enum}
\end{figure}

Ainsi, un message $m$ est convergent-stable si pour l'énumération de pairs $\tuple*{p_1, \dotsc}$ généré par l'algorithme de la \autoref{fig:all-perm-enum} auquel est passé en paramètre l'ensemble ordonné $\trm{invited}_J$, il existe une chaîne $m_1 \depeq \dotsc \depeq m_n$ issue du chemin parcouru qui prend source en $m$ tel que le $j$-ème pair de l'énumération est lié au $j$-ème message de la chaîne par l'une des relations suivantes~: \begin{inlinelist}
    \item le pair est l'auteur du message
    \item le pair est reconnu mal-intentionné dans le message
    \item le pair est invité dans le message
\end{inlinelist}.

L'algorithme de la \autoref{fig:convergent-stable} implémente cette définition avec une petite optimisation.
...
Cette construction peut être coûteuse sauf pour lez messages qui dépendent de tous les messages qui ont été précédemment ajoutés dans le journal.
...
Lorsque le message n'est pas un message du possesseur du journal nous pouvons pouvons toutefois retirer les pairs reconnus mal-intentionnés dans l'état $I$.
...
\textbf{require} $x \in J$
    ...
    \While{$j \leq n \land k \leq \trm{length}(J)$}
        \If{$y_k = x \lor \trm{id}(prev) \in \trm{deps}_{y_k}$}\Comment{$y_k$ fait-il partie du chemin parcouru~?}
...
\EndFunction
\end{algorithmic}
\caption{Algorithme qui détermine si un message $x$ est convergent stable dans $J$.}\label{fig:convergent-stable}
\end{figure}

Le pair créé d'abord un journal temporaire $Z$ dont il initialise les structures de données dépendantes à l'aides des structures de données dépendantes de l'état $I$ (ligne 2 à 4).
...
Si l'énumération de pairs est entièrement parcourut, le message $m$ est convergent-stables.
Nous avançons d'un pas dans l'énumération de pairs si et seulement si le pair courant est \begin{inlinelist}\item l'auteur du message courant, ou si il est \item le pair invité dans le message courant (le message courant est une invitation), ou si il est \item reconnu mal-intentionné dans $Z$ (il est présent dans l'ensemble $\trm{knwonMalicious}_Z$) \end{inlinelist}.
Dès lors que nous n'avançons plus dans l'énumération, nous progressons d'un pas sur le chemin de messages.
