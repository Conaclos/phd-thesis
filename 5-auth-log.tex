...
Un message $m$ d'invitation est accompagné des même méta-données qu'un autre message et d'une méta-données supplémentaire~:
\begin{description}
    \item[$\trm{peer}_m$:] l'identifiant de l'auteur de $m$ 
    \item[$\trm{num}_m$:] le numéro de $m$
    \item[$\trm{deps}_m$:] l'ensemble des dépendances déclarés de $m$
...
En revanche, nous ajoutons des conditions pour restreindre la définition de ce qu'est un message est bien-formé.
...
Un message $m$ devient stable dans un journal $J$ lorsqu'il n'est plus possible d'ajouter dans $J$ un message qui ne dépend pas de $m$.
Par-ce-que un pair mal-intentionné est capable de produire des messages non-linéaires, le concept de stabilité traite différemment les pairs mal-intentionné et honnêtes.
Chaque pair suspecte les autres pairs d'être mal-intentionné.

Lorsqu'un nouveau pair honnête rejoint la collaboration il récupère le journal d'un pair existant.
Si le journal est tronqué, le nouveau pair doit être capable de vérifier que le journal est correctement tronqué.

Ce nouveau pair suspecte également le pair 
...
Les journaux maintenus par le protocole à journaux tronqués ont donc des exécutions abstraites qui respectent le modèle de cohérence \emph{DynVFJC}.


Déterminer si un message est convergent-stable est couteux.


Déterminer l'ensemble des messages convergent-stables du journal $J$ est couteux.
...
\Procedure{$\trm{attemptTruncation}_i$}{}
    ...
    \If{$\trm{convergentStable}(m_k)$}
        ...
        \While{$j < k \land \nexists l > k \qsep \trm{id}(m_j) \in \trm{deps}_{m_l} $}
            \State $J \assign J \sminus m_j$ \Comment{supprime $m_j$ du journal $J$}
...
\begin{figure}[ht]
\begin{algorithmic}[1]
\Function{$\trm{allPermSeq}$}{$\tuple*{p_1, \dotsc, p_n}$}
    \If{$n = 1$}
...
\EndFunction
\end{algorithmic}
\caption{Algorithme de génération d'une énumération qui contient toutes les permutations d'un ensemble ordonné de pairs $\tuple*{p_1, \dotsc, p_n}$.}
\end{figure}
...
\begin{figure}[ht]
\begin{algorithmic}[1]
\Function{$\trm{convergentStable}$}{$x$}
    ...
    \State \textbf{let} $\tuple*{p_1, \dotsc, p_n} \assign \trm{allPermSeq}(\trm{invited}_J)$
    \State \textbf{let} $j \assign 1$
    \State \textbf{let} $\tuple*{m_1, \dotsc, m_k} \assign J$
    \State \textbf{let} $i \assign 1$
    \State \textbf{let} $\trm{prev} \assign x$
    \While{$j \leq n \land i \leq k$}
        \If{$x_i = x \lor \trm{id}(prev) \in \trm{deps}_{x_i}$}\Comment{$m_i$ fait partie du chemin parcouru}
            \If{$\trm{peer}_{m_i} = p_j \lor m_i = \tuple*{\trm{invite}, p_j} \lor p_j \in \trm{knownMalicious}_Z$}
                \State $j \assign j + 1$ \Comment{avance d'un pas dans l'énumération de pair}
            \Else
                \State $\trm{prev} \assign m_i$
                \State $Z \assign Z \lappend \tuple*{x_i}$
                \State $i \assign i + 1$\Comment{avance d'un pas dans le journal}
            \EndIf
        \Else
            \State $Z \assign Z \lappend \tuple*{m_i}$
            \State $i \assign i + 1$\Comment{avance d'un pas dans le journal}
        \EndIf
    \EndWhile
    \State \textbf{return} $j = n$
\EndFunction
\end{algorithmic}
\caption{Algorithme qui détermine si un message $x$ est convergent stable dans $J$.}
\end{figure}
