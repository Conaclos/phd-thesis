...
\subref{fig:authority-2} Le serveur intègre leurs modifications et transmet la nouvelle version du dessin aux collaboratrices.}
\label{fig:authority}
\end{figure}
...
\begin{subfigure}{.5\linewidth}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:optimisqtic-replciation-scenario-1}
\end{subfigure}%
\begin{subfigure}{.5\linewidth}
\centering
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:optimisqtic-replciation-scenario-2}
\end{subfigure}
...
\subref{fig:optimisqtic-replciation-scenario-1} Alice et Bea modifient indépendemment leur copie du dessin.
...
Les copies convergent~: elles obtiennent ainsi le même dessin.}
\label{fig:optimisqtic-replciation-scenario}
\end{figure}
...
Ils sont directement connectés les uns aux autres et partagent les mêmes responsabilités\footnote{Certaines infrastructures pair-à-pair peuvent présenter des pairs privilégiés qui possèdent des responsabilités supplémentaires ou spécifiques.%L'infrastructure reste décentralisée car plusieurs pairs peuvent avoir les mêmes privilèges.
}.
...
Le caractère distribué des infrastructures pair-à-pair de collaboration donne plus de pouvoir aux pairs.
Elles peuvent en particulier nuire au bon déroulement de la collaboration.
...
Pour ce faire, les pairs répliquent le contenu partagé et modifient leurs copies respectives.
...
Dans la~\autoref{fig:optimisqtic-replciation-scenario}, les copies divergent avant la synchronisation.
...
En effet, si deux contributeur·ice·s ont des copies qui ne sont pas capables de converger, leur vue du contenu sera différente.
...
Ce qui risque de retarder, voire empécher, la tenue de l'événement.

Certains pairs peuvent trouver un intérêt à compromettre le succès d'une collaboration.
Dans le cas de notre précédent exemple, il peut s'agir d'un opposant à la tenue de l'événement.
...
\begin{subfigure}{.5\linewidth}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:equivocation-scenario-1}
\end{subfigure}%
\begin{subfigure}{.5\linewidth}
\centering
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:equivocation-scenario-2}
\end{subfigure}
...
L'objectif de Mallory est atteint~: les copies de Alice et Bea demeurent divergentes.}
\label{fig:equivocation-scenario}
\end{figure}
...
L'état d'une copie est généralement plus faible que le journal d'opérations.
...
En présence de pairs mal-intentionnés, des états peuvent être falsifiés.
Nous proposons un protocole pour tronquer un journal authentifié et authentifier un état à l'aide d'un journal tronqué.
Pour tronquer le journal nous reposons sur un concept de stabilité.
...
%Nous proposons un mécanisme pour tronquer un journal authentifié sans compromettre sa cohérence et sa convergence.
%Pour ce faire nous définissons un concept de stabilité et un nouveau modèle de cohérence de donnée basé sur \acl{VFJC}.
%Notre concept de stabilité étend \ac{CS} et permet de s'assurer qu'une opération a été observé par l'ensemble des pairs dans un groupe dont la composition peut évoluer dans le temps et peut inclure des pairs mal-intentionnés.
%Une nouvelle pair n'est pas capable d'obtenir l'état du contenu partagé à partir d'un journal élagué.
%Nous proposons un mécanisme pour authentifier un état à partir d'un journal tronqué qui lui est attaché.


\subsection{Protocole de réplication pour la co-édition de texte}

Les activités de collaboration, selon leur nature, requièrent différents niveaux de disponibilité, de latence, et de passage à l'échelle.
L'édition collaborative de texte présente un défi car elle requiert à la fois une haute disponibilité, des latences basses, et un passage à l'échelle raisonnable.
Elle permet à des collaborateur·ice·s géographiquement éloigné·e·s de co-rédiger un texte simultanément ou de manière différée dans le temps.

Son usage s'est répandue grâce à des éditeurs de texte collaboratifs tel que \emph{EtherPad}\footnote{\href{https://etherpad.org}{etherpad.org}}, \emph{Google Docs}\footnote{\href{https://docs.google.com}{docs.google.com}}, et \emph{ShareLaTeX / Overleaf}\footnote{\href{https://www.overleaf.com}{www.overleaf.com}}.
...
Pour dépasser ces limitations, la communauté scientifique a proposé plusieurs protocoles de réplication optimiste qui prennent avantage des infrastructures pair-à-pair~\autocite{ahmednacer2011evaluatingcrdts}.
...
En pratique, la couche de livraison d'opérations ne prend pas en compte les ordres spécifiques d'intégration de chaque protocole.
En général, une livraison causale des opérations est utilisée~: si un pair intègre un ensemble d'opérations, alors cet ensemble est intégré sur toute copie avant l'intégration des opérations ultérieures de ce pair.
...
\begin{subfigure}{.5\linewidth}
    ...
    \end{tikzpicture}
    \caption{}
    \label{fig:op-integration-order-example-1}
\end{subfigure}%
\begin{subfigure}{.5\linewidth}
\centering
    \begin{tikzpicture}
...
%Les protocoles de réplication optimiste se déclinent en deux familles~: les protocoles synchronisés par opérations et les protocoles synchronisés par états.
%La première famille représente les modifications effectuées sur une copie par des opérations.
%Les pairs synchronisent leur copie en échangeant leurs opérations.
%Dans la seconde famille, les pairs s'échangent directement l'état de leur copie ou une différence d'état.
%Ils intègrent les modifications des autres pairs en fusionnant l'état actuel de leur copie avec les états ou les différences d'états qu'ils reçoivent.
%La~\autoref{fig:protocol-kind} illustre la différence entre ces deux familles de protocoles.
...
%        \node[
%            label=below:{Alice}
%        ] (A) at (0,0) {\includegraphics[scale=0.6]{collab/device.pdf}};
%        \path (A)
%            to +(4,0) node[
%                label=below:{Bea}
%            ] (C) {\includegraphics[scale=0.6]{collab/device.pdf}};
%        % links
%        \draw[link,-latex] (A) to node[midway] {\includegraphics[scale=0.5]{collab/contrib-a1.pdf}} (C);
%    \end{tikzpicture}
%    \caption{}
%    \label{fig:protocol-kind-1}
%\end{subfigure}%
%\begin{subfigure}{.5\linewidth}
%    \centering
%    \begin{tikzpicture}
...
%            ] (C) {\includegraphics[scale=0.6]{collab/device.pdf}};
%        % links
%        \draw[link,-latex] (A) to node[midway] {\includegraphics[scale=0.5]{collab/doc-a1.pdf}} (C);
%    \end{tikzpicture}
%    \caption{}
%    \label{fig:protocol-kind-2}
%\end{subfigure}
%\caption[Type de synchronisation des protocoles de réplication optimiste]{Alice et Bea répliquent de manière optimiste un dessin.
%Alice ajouté un carré sur le dessin et transmet sa modification à Bea.
%La manière de représenter et donc de propager sa modification dépend de la famille à laquelle le protocole de réplication appartient.
%\subref{fig:protocol-kind-1} Un protocole à base d'opérations transmet la modification de Alice sous forme d'une opération.
%\subref{fig:protocol-kind-2} Un protocole à base d'état transmet la nouvelle version du dessin.}
%\label{fig:protocol-kind}
%\end{figure}
%
%% Les protocoles à base d'opérations
%Les protocoles synchronisés par opérations supposent une livraison fiable et causale des opérations.
%Chaque opération doit être livrée exactement une fois à chaque pair et doit être livrée après les opérations qui la précédent.
%L'intégration d'une opération est donc retardée jusqu'à l'intégration des opérations précédentes.
%La perte d'opérations sur le réseau ou la déconnexion de pairs peut donc propager des ralentissement dans l'ensemble du système~\autocite{alvisi_2017_writes-dirty-secret}.
%Les protocoles à base d'opérations souffrent également d'un coût en communication et en ressources de calcul lorsque le nombre d'opérations à échanger et à intégrer est important.
%Cette situation est rencontrée lorsque deux partitions de pairs n'ont pas synchronisées leurs copies depuis un certain temps.
%
%% Parler des avantages de l'op-based ?
%
%% Les protocoles à base d'état
%Contrairement aux protocoles synchronisés par opérations, les protocoles synchronisés par états ont des hypothèses plus faibles sur la livraison des états ou des différences d'états.
%Ce qui laisse plus de choix dans la conception de ces protocoles.
%Certains protocoles synchronisés par états sont ainsi plus efficaces que les protocoles synchronisés par opérations~\autocite{enes_2018_efficient-sync-state-based-crdt}.
%
%% Les protocole à base de différences d'état
%
%% Édition collaborative seulement sur des protocoles à base d'opérations
%% Est-il possible d'avoir des protocoles à base de différences ?
%Plusieurs protocoles synchronisés par opérations ont été proposés pour l'édition collaborative en simultané de texte et de séquences.
%A l'heure actuelle, aucun protocole synchronisés par états n'a été proposé.
%Ce qui nous amène à la deuxième question de recherche que nous explorons dans ce manuscrit~:
%\textbf{Est-il possible de concevoir un protocole synchronisé par états pour l'édition collaborative en simultané de texte~?}
...
Il n'y a pas d'intégrations d'états qui sont retardées.
...
Cependant, il privilégie l'envoie de différences d'états qui synthétise une ou plusieurs modifications.
Les différences d'états peuvent être intégrés plusieurs fois dans des ordres arbitraires.
...
Nous proposons donc d'adopter ce schéma de synchronisation pour l'édition collaborative de texte.
Pour ce faire, nous concevons un nouveau protocole de réplication optimiste nommé \emph{Dotted LogootSplit}.


\section{Organisation du manuscrit}

Le reste du manuscrit est organisé en 6 chapitres :
...
Le protocole à journaux tronqués s'approche d'une réponse affirmative à notre première question de recherche.
...
Nous présentons Les types de données répliquées à l'aide d'un modèle unifié pour mieux comprendre ce qui les différencient.
A l'aide de ce même modèle nous répondons à notre deuxième question de recherche en proposant \emph{Dotted LogootSplit}, le premier type séquence de données répliquées synchronisé par différence d'états.
